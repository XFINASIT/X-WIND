...
mbc.EigenVects_ori    = origEigenVects;

real_Evals = real(mbc.Evals);
imag_Evals = imag(mbc.Evals);

real_Evals_ori = real(origEvals);
imag_Evals_ori = imag(origEvals);


mbc.NaturalFrequencies = sqrt( real_Evals.^2 + imag_Evals.^2 );
mbc.NaturalFrequencies_ori = sqrt( real_Evals_ori.^2 + imag_Evals_ori.^2 );
...
mbc.NaturalFreqs_ori_Hz = mbc.NaturalFrequencies_ori./(2*pi);
