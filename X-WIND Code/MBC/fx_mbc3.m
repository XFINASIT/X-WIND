...
FileNames = "primary1.1.lin";
mkdir MBC_results;
fileID_out = fopen('MBC_results\results.txt','w');
...
fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.A,1);
size_a2 = size(MBC.A,2);
fprintf(fileID_out,'---> MBC A\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.A,1);
    fprintf(fileID_out,'%5d\t',MBC.A(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.B,1);
size_a2 = size(MBC.B,2);
fprintf(fileID_out,'---> MBC B\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.B,1);
    fprintf(fileID_out,'%5d\t',MBC.B(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.C,1);
size_a2 = size(MBC.C,2);
fprintf(fileID_out,'---> MBC C\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.C,1);
    fprintf(fileID_out,'%5d\t',MBC.C(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.D,1);
size_a2 = size(MBC.D,2);
fprintf(fileID_out,'---> MBC D\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.D,1);
    fprintf(fileID_out,'%5d\t',MBC.D(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.AvgA,1);
size_a2 = size(MBC.AvgA,2);
fprintf(fileID_out,'---> MBC AvgA\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.AvgA,1);
    fprintf(fileID_out,'%5d\t',MBC.AvgA(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.AvgB,1);
size_a2 = size(MBC.AvgB,2);
fprintf(fileID_out,'---> MBC AvgB\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.AvgB,1);
    fprintf(fileID_out,'%5d\t',MBC.AvgB(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.AvgC,1);
size_a2 = size(MBC.AvgC,2);
fprintf(fileID_out,'---> MBC AvgC\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.AvgC,1);
    fprintf(fileID_out,'%5d\t',MBC.AvgC(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.AvgD,1);
size_a2 = size(MBC.AvgD,2);
fprintf(fileID_out,'---> MBC AvgD\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.AvgD,1);
    fprintf(fileID_out,'%5d\t',MBC.AvgD(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.Evals,1);
size_a2 = size(MBC.eigSol.Evals,2);
fprintf(fileID_out,'---> MBC Evals\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.Evals,1);
    fprintf(fileID_out,'%f + %fi\t',real(MBC.eigSol.Evals(jj,:)),imag(MBC.eigSol.Evals(jj,:)));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.EigenVects,1);
size_a2 = size(MBC.eigSol.EigenVects,2);
fprintf(fileID_out,'---> MBC EigenVects\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.EigenVects,1);
    fprintf(fileID_out,'%f + %fi\t',real(MBC.eigSol.EigenVects(jj,:)),imag(MBC.eigSol.EigenVects(jj,:)));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.NaturalFreqs_Hz,1);
size_a2 = size(MBC.eigSol.NaturalFreqs_Hz,2);
fprintf(fileID_out,'---> MBC NaturalFreqs_Hz\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.NaturalFreqs_Hz,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.NaturalFreqs_Hz(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.NaturalFrequencies,1);
size_a2 = size(MBC.eigSol.NaturalFrequencies,2);
fprintf(fileID_out,'---> MBC NaturalFrequencies\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.NaturalFrequencies,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.NaturalFrequencies(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.DampRatios,1);
size_a2 = size(MBC.eigSol.DampRatios,2);
fprintf(fileID_out,'---> MBC DampRatios\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.DampRatios,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.DampRatios(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.DampedFrequencies,1);
size_a2 = size(MBC.eigSol.DampedFrequencies,2);
fprintf(fileID_out,'---> MBC DampedFrequencies\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.DampedFrequencies,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.DampedFrequencies(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.DampedFreqs_Hz,1);
size_a2 = size(MBC.eigSol.DampedFreqs_Hz,2);
fprintf(fileID_out,'---> MBC DampedFreqs_Hz\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.DampedFreqs_Hz,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.DampedFreqs_Hz(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.MagnitudeModes,1);
size_a2 = size(MBC.eigSol.MagnitudeModes,2);
fprintf(fileID_out,'---> MBC MagnitudeModes\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.MagnitudeModes,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.MagnitudeModes(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.PhaseModes_deg,1);
size_a2 = size(MBC.eigSol.PhaseModes_deg,2);
fprintf(fileID_out,'---> MBC PhaseModes_deg\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.PhaseModes_deg,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.PhaseModes_deg(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.NaturalFreqs_ori_Hz,1);
size_a2 = size(MBC.eigSol.NaturalFreqs_ori_Hz,2);
fprintf(fileID_out,'---> MBC NaturalFreqs_Original_Hz\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.NaturalFreqs_ori_Hz,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.NaturalFreqs_ori_Hz(jj,:));
    fprintf(fileID_out,'\n');
end

fprintf(fileID_out,'\n'); % blank
size_a1 = size(MBC.eigSol.NaturalFrequencies_ori,1);
size_a2 = size(MBC.eigSol.NaturalFrequencies_ori,2);
fprintf(fileID_out,'---> MBC NaturalFrequencies_Original\n');
fprintf(fileID_out,'---> %5d %5d',size_a1,size_a2);
fprintf(fileID_out,'\n'); % blank
for jj = 1:size(MBC.eigSol.NaturalFrequencies_ori,1);
    fprintf(fileID_out,'%5d\t',MBC.eigSol.NaturalFrequencies_ori(jj,:));
    fprintf(fileID_out,'\n');
end

% fprintf(fileID_out,'%5d',MBC.A);

%   disp('  ');
%   disp(' Multi-Blade Coordinate transformation completed ');
%-----------------------------------------------------------

%campbell_diagram_data(MBC, BladeLen, TowerLen, xlsFileName)
%return;
end
