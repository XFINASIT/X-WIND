...
C     -------------------------------------------------------
      ...
      ! SKIP FOR WAVE RESPONSE SPECTRUM
...
C     ------------------------------
      IF(IFLOOR.EQ.3) THEN ! FOR LINK CONSTRAINS BY BJ
      CALL NODEIN2 (IA(LID),A(LSS),A(LSC),IA(LDS),A(LDC),A(LXY),
...
C     READ LINK CONSTRAINTS DATA FOR LINK CONSTRAINS BY BJ ---------------------------
      IF(IFLOOR.EQ.3) THEN
      
      CALL ILINKCON

C     CALCULATE THE CENTER NODE IN EACH FLOOR FOR LINK CONSTRAINS BY BJ
      CALL CALCOORD(DIAPH(JCO))
      CALL CALCENTRO(DIAPH(JCO),DIAPH(JCXY),IDIAPH(JIF),IDIAPH(JNUM),IDIAPH(JNED))
      
      IF(IEQNUM.EQ.1) THEN
         CALL CENTERMASS(DIAPH(JCXY),DIAPH(JMCO)) 
      ENDIF  
...
C	READ GENERAL LINK ELEMENT SONGSAK JUN2007
	IF(IFLOOR.EQ.3) THEN !FOR LINK CONSTRAINTS BY BJ
	  CALL INPLINK2(IA(LCH),0)
	ELSE
	  CALL INPLINK(IA(LCH),0) ! ORIGINAL 
      ENDIF
...
C	OFFSHORE LOAD ANALYSIS
	...
	IF(KOFFL.EQ.1.OR.KSPEC.EQ.1) THEN
	  ...
	  CALL OFFSHFILE(NEQ) !OPEN FILE TO STORE OFFSHORE LOAD
        CALL RANDOM_DATA
        CALL MARINE_GROWTH ! READING THE MARINE GROWTH PARAMETER
C     PRINT WATER SURFACE ELEVATION PROFILE
      CALL PRINT_WATER_SURFACE_ELEVATION (A(LXY))
      ENDIF
      CALL DIROPEN_OFFSHORE_BINARY
      
      IF(KDESIGN.EQ.1)THEN
       CALL DESIGNREAD
      ENDIF

C     OUTPUT TO FAST SOFTWARE   
      !CALL COORDINATE_TRANSFER   
      !CALL BUNDARY_CONDITION_TRANSFER
      !CALL CONNECTIVITY_FRAME_FAST
...
C	LOAD COMBINATION DATA
	IF(ISOLOP.EQ.1.AND.ICONTROLSPEC.EQ.0) CALL LDCMREAD
      IF(ISOLOP.EQ.1.AND.ICONTROLSPEC.EQ.1) CALL LDCMREAD
      IF(ISOLOP.EQ.21) CALL LDCMREAD
...
C	ADDED MASS !MUST BE FOLLOW LOCATI (7)
	CALL NODALMS(A(LCM),IA(LID))  
      CALL MARINE_GROWTH_MASS (IA(LEL),A(LCM),IA(LID))
...
C	IF (ISOLOP.EQ.3.OR.ISOLOP.EQ.11) GOTO 100	!removed 24Feb06
	...
	IF (ISOLOP.EQ.5.OR.ISOLOP.EQ.1.OR.ISOLOP.EQ.16) THEN
          IF(KSPEC.EQ.1) GOTO 100
      ENDIF
      
      IF (IFLOOR.NE.0) CALL WIND_ASCE7_02 ! LATERAL WIND LOAD
...
C     READ EQUATION SIZE & SOLUTION OF:	
	...
	IDOF(1:9) = 0
C     LINE NUMBER 4 	
	READ (ITI,*) ISOLOP,NRE,NSN,NEG,NBS,NLS,NSS,NLSS,NMSTIFF,
	1			 NLA,LCS,LGEN,IOFFL,LC107,LC207,IDOFB,IREDU,MAXCON,
	2			 KPOSD,IMOPT,ITPRE,NGRAV!,IFLOOR !,LSYMM
      ! IFLOOR = NUMBER OF FLOOR FOR BUILDING CODE
      !IFLOOR = 0
 	LSYMM = 0
 	
 	IMOPT = 1
 	ITPRE = 3
      IORI_OFFSHORE  = 0.     ! OFFSHORE LOADCASE
      NPRINTREACTION = ISOLOP
      
C     --- LINK CONSTRAINTS SET : DEFINE THE NUMBER OF FLOOR ---
      IF(IFLOOR.EQ.3) THEN 
        READ (ITI,*)
        READ (ITI,*) NFLOOR
        
        NSN0 = NSN ! SAVE ORIGINAL NSN INTO NSN0
        NSNL = NSN + NFLOOR ! ADD THE NUMBER OF CENTER NODE IN EACH FLOOR
        NSN  = NSNL
      ENDIF
...
!       WRITE (*,700)
! 700   FORMAT ("**************************************************")
! 701   FORMAT ("THE MODEL HAS MORE THAN 1000 NODES.")
! 702   FORMAT ("PLEASE CONTRACT DEVELOPER FOR GET THE LICENCE VERSION.")
! 703   FORMAT ("")
!       STOP
!       ENDIF
...
C	PREPARE STORAGE FOR ELEMENT GROUP WORKING ARRAY (NEG,10)
	CALL  DEMINT('GWOK',KGWOK,NEG,10)

	CALL  DEMINT('GEAS',KGEAS,NEG,15)

C	SYSTEM MATRIX SOLVER SCHEME
	CALL  DEMINT('SOLV',KSOLV,  1, 5,  )		!STORE SOLVER OPTION
	CALL MINTFIL('SOLV',IMOPT,  1, 1, 1)		!STIFFNESS PROFILE AND SOLVER OPTION --0 = DIRECT  , 1 = ITERATION 
	CALL MINTFIL('SOLV',ITPRE,  1, 2, 1)		!PRECONDITIONER SCHEME FOR ITERATION --0 = STANDARD, 1 = JACOBI   , 2 = IMCOMPLETE CHOLESKI
...
C	-----------------------------------------
1392  IF (NSOIL.EQ.1) CALL READSOILDATA ("READ",0,0,0,0,0,0,0,0)
...
C     ----------------------------------------
 ...
 590  CONTINUE
      ...
      IF (ISOLOP.EQ.21) NBOUNDARY = ID
      
      CALL READADDEDSTIFFNESS
C
      RETURN
C
...
C     -------------------------------
	...
	READ (ITI,*) ITYPE,ISTYP,NLOPT,MTMOD,NELE,NMPS,NGPS,
     1             NOPS,NHIGE,NMV,NMP,NGP,NNM,NGR,NGS,NGT
      IF (ISOLOP.EQ.2.AND.NLOPT.EQ.2) NLOPT = 3 
...
C	======================================
	...
	CASE(5)
	CALL OFPROP (A(LOP),IA(LOS),IA(LGID),NOPS,NELE,"READ")
...
c     LOCAL AXES TO THE ELEMENT ------------ SOLID-SHELL(P) BY BJ
      
      
      IF(NLA.NE.0) THEN
       IF(ITYPE.EQ.10.AND.ISTYP.EQ.1) THEN 
        CALL LOCALREAD(IA(LGS))
C      
C      ALLOCATE (LOMAT(NELE,2))
C          
C          READ(ITI,*)
C          
C          DO I = 1,NELE
C              READ(ITI,*) IELLO,NLOCAL
C               LOMAT(I,1) = IELLO
C               LOMAT(I,2) = NLOCAL
C          ENDDO
C         
...
C     Sorn Local Corrdinate Start
      CALL OFPROP (A(LOP),IA(LOS),IA(LGID),NOPS,NELE,"WRIT")
C     Sorn Local Corrdinate End
...
C	SET NOLIN TO BE 1  SO THAT IT WILL ACTIVATE AN ITERATION OPTION IN LINEAR ANALYSIS
	IF(ISOLOP.EQ.1) THEN
	IF(ISTYP.EQ.4.OR.ISTYP.EQ.6.OR.ISTYP.EQ.7.OR.ISTYP.EQ.8) NOLIN = 1
	ENDIF
...
C      WRITE (NELEMA) ( A(NLNU),NLNU=LMP,LMP+(LEST(NEG+IEG)-1))
...
C	------------------------------------------------------------
	...
	NPT = NGR*NGS*NGT
	IF(ISTYP.EQ.4) NPT = NGR*NGS !FOR SOLID-SHELL(LIU) BY BJ
      !IF(ISTYP.EQ.1) NPT = NGR*NGS !FOR SOLID-SHELL BY BJ
...
C     FOR LINK CONSTRAINTS BY BJ
	...
	COMMON /MEMBA/ JMEMBT(5000,3),JMEMBS(5000,3),JMEMBF(5000,3), !SAVE ELEMENT NUMBER FOR TRUSS & SHELL FOR DIAPHRAGM BY BJ
     ...
     2                NELET,NELES,NELEF       
      DIMENSION PROPM2(NMP,2) ! FOR DIAPHRAGM CONSTRIANTS BY BJ
C	==================================================================      
...
c-----composite
 100  IF (MTMOD.EQ.2) Go to 10
      ...
      CALL PROPERTY_TEMPORY_FAST(ISET,PROP)
C     FOR LINK CONSTRAINTS BY BJ      
      IF(IFLOOR.EQ.3) THEN
        YOUNGMAT = PROP(1)
        TEMAT = TEMAT + YOUNGMAT
        IEMAT = IEMAT + 1
        IF(YOUNGMAT.GT.EMAXM) EMAXM = YOUNGMAT
      ENDIF
C     ----------------------------------------
      GO TO 10000
...
C     ---------------------------------------
 293  IF (PROMPT)  WRITE (ITO,3000)  NMPS,NELE
      IF (PROMPT)  WRITE (10,3000)  NMPS,NELE
      IELE = 0
	IELEt = 0
...
c	read ASSIGN MATRIAL PROPERTIES TO ELEMENTS ***********************************
  ...
  300	READ (ITI,*,END=10003)  MEMGID,ISET
      CALL PROPERTIES (PROP,MEMGID,ISET,IMP,ITYPE)
...
C	=======================================================
 ...
 2000 FORMAT (//21X,35HMATERIAL PROPERTIES FOR SET NUMBER ,I2/
     1         21X,37(1H-)//
     2     14X,6H PROP(,I2,32H)  YOUNG'S MODULUS . . . .  YM =,E14.6/
     3     14X,6H PROP(,I2,32H)  POISSON'S RATIO . . . .  PR =,E14.6/
     4     14X,6H PROP(,I2,32H)  YIELD STRESS  . . . . . YLD =,E14.6/
     5     14X,6H PROP(,I2,32H)  STRAIN HARDENING MODULE  HP =,E14.6/
     6    (14X,6H PROP(,I2,32H)  . . . . . . . . . . . . . . =,E14.6))
C***+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C     READ(ITI,*) IANUR,(INCI(II),II=1,NNM)  ! INCI = ELEMENT CONNECTIVITY 
      READ(ITI,*) IANUR,INCI(1:NNM)
      
      !CALL STORENODE (IANUR,INCI)
...
C	==================================================================
	...
	COMMON /GiDEle/ LGID ! added by de silva 2004/6/28
      COMMON /LOCO/ LOP,LOS,LSS,LSS2,LSS3,LHG,LHGN
      COMMON /FAST_TP/ NODEFAST,NPOINT(100)
...
100   READ (ITI,*) NNLO,NLEG,NSTTM,NEGLO,NLEOFF,NLEGOFF,NSETLOOFF,NEXTERNAL_GEN,NEXTERNAL_OFF!,NBCENT ! ,NODSPEC
      !NBCENT = 0
      ICLO = NBCENT
...
C     -------------------------------------------------------------------------------------
C     GENERAL LOADCASE WITH THE FIRST START OF ANALIZE, THEN FOLLWING BY OFFSHORE LOADCASE  
C     EXAMPLE : THE MODEL HAS INCLUDING GENERAL LOADCASE 1-2 AND OFFSHORE LOADCASE 1-4
C               TOTAL OF LOADCASE (LCS) = 6 
C               GENERAL LOADCASE 1   : ILC = 1
C               GENERAL LOADCASE 2   : ILC = 2
C               OFFSHORE LOADCASE 1  : ILC = 3
C               OFFSHORE LOADCASE 2  : ILC = 4
C               OFFSHORE LOADCASE 3  : ILC = 5
C               OFFSHORE LOADCASE 4  : ILC = 6
C     -------------------------------------------------------------------------------------
      
C     -----------------------------
C     READ AND GENERATE NODAL LOADS
C     -----------------------------
      IF (NNLO.GT.0)   CALL NODELO (IA(LID),NSF,NNLO)            ! GENERAL LOADCASE
      IF (NLEOFF.GT.0) CALL NODELO_OFFSHORE (IA(LID),NSF,NLEOFF) ! OFFSHORE LOADCASE
      IF (KFAST.EQ.2)  CALL FASTTOPFORCE (IA(LID),NSF,NPOINT(1),NUMCASE,NFASTPARA,NTFASTPARA,R,NEQ,1,'WRIT')
      IF (ICLO.GT.0)   CALL NODELO2 (IA(LID),NSF,NNLO,ICLO)      ! FOR CENTER LOAD BY BJ

C	SETTLEMENT SONGSAK JUN2006
	IF (NSTTM.GT.0) CALL INPSET (NSTTM)
      IF (NSETLOOFF.GT.0) CALL INPSETOFF (NSETLOOFF)
...
C      READ (NELEMA) ( A(NLNU),NLNU=LMP,LMP + LEST(IEG+NEG)-1)
	...
	IF(ITYPE.EQ.5) CALL SMHZERO                      !INITIALIZE ZERO SMOOTH INTERPOLATION OF BEAM&FRAME SONGSAK FEB2007	
      IF(ITYPE.EQ.5) CALL SMHZERO_OFFSHORE             !INITIALIZE ZERO SMOOTH INTERPOLATION OF BEAM&FRAME SONGSAK FEB2007	
	ENDDO
C     -----------------
...
C     -------------------------------------------------------
      IF (NLEG.EQ.0.AND.NLEGOFF.EQ.0) GOTO 933
...
400   MFRMLD = 0
	...
	MFRMLD(1:5) = [NWLO,NBODY,NTMPLD,NOFFL,NBOUN]
...
C     -------------------------------------------------------
500   IF(ITYPE.EQ.2) THEN !ADDED HERE FOR NEW TRUSS&CABLE ELEMENT LOAD SONGSAK MAY2007
C	NWLO,NBODY,NTMPLD
      
      ! GENERAL LOAD
      IF (NLEG.EQ.0.OR.NLOAD_OFF.EQ.1) GOTO 550

	IF (NWLO.NE.0) THEN
	READ(ITI,*)
	CALL SEFTRS (NWLO,A(LCO),IA(LGS),A(LGP),
	1			 IA(LLM),A(LMP),IA(LMS),IA(LGID),"GEN") 
	ENDIF
	IF (NBODY.NE.0) THEN
	READ(ITI,*)
...
!      
!      IF (NBOUN.NE.0)THEN
!      CALL CABLE_OFFSHORE_BOUN (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),
!	1		           IA(LGID),MFRMLD,IA(LHG),IA(LHGN)) 
!      ENDIF
...
550   IF (NLEGOFF.NE.0.AND.NLOAD_OFF.EQ.1)THEN 
      ! OFFSHORE LOAD
      !IF (NLEG.NE.0)THEN
      !READ (ITI,*) ! READ HEAD
      !READ (ITI,*) ! ELEMENT LOOP
      !ENDIF
      ! OFFSHORE LOAD UPDATE 09/2020
      READ (ITI,*) NWLO_OFF,NWAVE,NCURRENT,NBOUN,NWAVECUR  ! INDEX DATA OF OFFSHORE LOADCASE
      
      IF (NWLO_OFF.NE.0) THEN
	READ(ITI,*)
	CALL SEFTRS (NWLO_OFF,A(LCO),IA(LGS),A(LGP),
	1			 IA(LLM),A(LMP),IA(LMS),IA(LGID),"OFF") 
      ENDIF
      
      IF (NWAVE.NE.0)THEN ! WAVE
      MFRMLD(1)  = NWAVE
      CALL CABLEOFFSHORE (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),
	1		           IA(LGID),MFRMLD,IA(LHG),IA(LHGN)) 
      ENDIF
      
      IF (NCURRENT.NE.0)THEN ! CURRENT
      MFRMLD(1)  = NCURRENT
      CALL CABLEOFFSHORE (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),
	1		           IA(LGID),MFRMLD,IA(LHG),IA(LHGN)) 
      ENDIF
      ...
      IF (NBOUN.NE.0)THEN
      CALL CABLE_OFFSHORE_BOUN (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),
	1		           IA(LGID),MFRMLD,IA(LHG),IA(LHGN),NBOUN) 
      ENDIF
      
      IF (NWAVECUR.NE.0)THEN
      MFRMLD(1)  = NWAVECUR
      CALL CABLEOFFSHORE (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),
	1		           IA(LGID),MFRMLD,IA(LHG),IA(LHGN)) 
      ENDIF
...
C     -------------------------------------------------------
	IF(ITYPE.EQ.5) THEN !ADDED HERE FOR NEW FRAME ELEMENT LOAD SONGSAK MAY2007
      ! GENERAL LOAD CASE
      IF (NLEG.EQ.0.OR.NLOAD_OFF.EQ.1) GOTO 600     
	CALL FRAMFOC (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		      MFRMLD,IA(LHG),IA(LHGN)) 
      
      
600   IF (NLEGOFF.NE.0.AND.NLOAD_OFF.EQ.1)THEN  
          
          
      ! OFFSHORE LOAD CASE BY TOEY 4/2018
      !IF (NLEG.NE.0)THEN
      !READ (ITI,*) ! READ HEAD
      !READ (ITI,*) ! ELEMENT LOOP
      !ENDIF
      ! INDEX DATA OF OFFSHORE LOADCASE, PLEASE SEE THE DETIAL IN TCL FILES
      !READ (ITI,*) NSELF,NUNI,NMARINE,NWAVE,NBUOY,NCURRENT,NBREAK,NHIROI,NGODA,NWIND ! INDEX DATA OF OFFSHORE LOADCASE
      READ (ITI,*) NSELF,NUNI,NMARINE,NWAVE,NBUOY,NCURRENT,NBREAK,NWIND,NWAVECUR ! INDEX DATA OF OFFSHORE LOADCASE
      MFRMLD     = 0.
      MFRMLD(1)  = NSELF    ! SELF WEIGHT
      MFRMLD(2)  = NUNI     ! DEAD LOAD OR UNIFORM LOAD
      MFRMLD(3)  = NMARINE  ! MARINE GROWTH 
      MFRMLD(4)  = NWAVE    ! WAVE LOAD
      MFRMLD(5)  = NCURRENT ! CURRENT LOAD
      MFRMLD(6)  = NBREAK   ! BREAKLING WAVE
      MFRMLD(7)  = NWIND    ! WAVE LOAD
      MFRMLD(8)  = NBUOY    ! BUOYANCY LOAD
      !MFRMLD(9)  = NHIROI
      !MFRMLD(10)  = NGODA 
      MFRMLD(11)  = NWAVECUR 
      
      
      IF (NSELF.NE.0.OR.NUNI.NE.0)THEN
      ! GENERAL LOAD FOR OFFSHORE LOAD CASE (SELF WEIGHT, DEAD LOAD AND ETC)
      CALL FRAMFOC_OFFSHORE (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		              MFRMLD,IA(LHG),IA(LHGN))    
      ENDIF
      ! READING MARINE GROWTH
      AMARINE = 0.
      GROWTH  = 0.0D0
      IF (NMARINE.NE.0)THEN
      ! TEMP STORAGE. WILL BE DELTETE AFTER CALCULATION
      ! THIS FILE FOR INCULDING MARINE GROWTH IN TERM OF WAVE, WIND AND CURRENT
      OPEN(UNIT=70,FILE='Marine Growth.dat'   ,STATUS='UNKNOWN')
      READ (ITI,*) 
        DO I = 1,NMARINE
        READ (ITI,*) MLE_MARINE,THICKNESS
        WRITE (70,"(I5,2X,E15.7)") MLE_MARINE,THICKNESS
        ENDDO
      ENDIF
      
      ! COMBINATION WAVE AND CURRENT VELOCITY 10/2018
      IF (NWAVE.NE.0..OR.NBUOY.NE.0.)THEN
      ! ONLY WAVE LOADING
      !CALL HIROI (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	!1            MFRMLD,IA(LHG),IA(LHGN))
      CALL FRAMFOC_OFFSHORE_WAVE (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		              MFRMLD,IA(LHG),IA(LHGN))
      ENDIF
      
      IF (NCURRENT.NE.0)THEN
      ! ONLY CURRENT LOADING
      MFRMLD       = 0.
      MFRMLD(1)    = NCURRENT
      MFRMLD(3)    = NMARINE
      MFRMLD(4)    = NWAVE
      ! MFRMLD(4)  = NWAVE  >> WAVE INDEX
      CALL FRAMFOC_OFFSHORE_CURRENT (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		              MFRMLD,IA(LHG),IA(LHGN))
      ENDIF
      ...
      MFRMLD(1)  = NBREAK
      MFRMLD(3)  = NMARINE
      CALL FRAMFOC_OFFSHORE_BREAKING (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		              MFRMLD,IA(LHG),IA(LHGN))
      ENDIF
      ...
      MFRMLD(1)  = NWIND
      MFRMLD(3)  = NMARINE
      CALL FRAMFOC_OFFSHORE_WIND (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		              MFRMLD,IA(LHG),IA(LHGN))
      ENDIF
	...
	1            MFRMLD,IA(LHG),IA(LHGN))      
      ...
      IF (NGODA.NE.0)THEN
      ! GODA FORMULAR
      CALL GODA (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1            MFRMLD,IA(LHG),IA(LHGN))      
      ENDIF
      ...
      MFRMLD(3)    = NMARINE
      MFRMLD(4)    = NWAVECUR
      CALL FRAMFOC_OFFSHORE_WAVE_CURRENT (IA(LLM),A(LCO),IA(LGS),A(LGP),IA(LMS),A(LMP),IA(LGID),
	1		              MFRMLD,IA(LHG),IA(LHGN)) 
      ENDIF
      ...
      !  STABILITY CHECK
      !CALL STABILITY (A(LCO),IA(LGID),IA(LID),NSF)
      
      
      CALL FIXSTOR(1,1,1,WAVFOC,14,1.0D0,'RED','VARY')
...
C	NWLO,NPLOE,NHLOE,NBODY,NTMPLD
      ...
      IF (NLEG.EQ.0.OR.NLOAD_OFF.EQ.1) GOTO 700    
	IF (NWLO.NE.0) THEN
	READ(ITI,*)
...
C	NWLO,NPLOE,NHLOE,NUGLO,NUPRE,NHYDR,NBODY,NTMPLD
	...
	CALL SHETRAC (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1              NGP,NCO*NNM,NEX,NEF,NGLOE,IA(LGID))
      ...
      ! OFFSHORE LOAD CASE BY TOEY 04/2018
700   IF (NLEGOFF.NE.0.AND.NLOAD_OFF.EQ.1)THEN  
      !READ (ITI,*) ! READ HEAD
      !READ (ITI,*) ! ELEMENT LOOP
      
      ! INDEX DATA OF OFFSHORE LOADCASE, PLEASE SEE THE DETIAL IN TCL FILES
      READ (ITI,*) NSELF,NWAVE,NCURRENT,NBREAK,NWIND,NWAVECUR  ! INDEX DATA OF OFFSHORE LOADCASE (SHELL ELEMENT)
      
      IF (NSELF.NE.0) THEN
      READ(ITI,*)    
      ! SELF WEIGHT FOR SHELL ELEMENT (THE STORAGE OF FORCE IS DIFFERENT IN GENERAL LOAD)
      CALL SEFSHS_OFF (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),A(LMP),IA(LMS),
     1             NGP,NCO*NNM,NEX,NEF,IA(LGID),NSELF)
      ENDIF
      
      IF (KSPEC.NE.1) THEN ! REGULAR WAVE
	    IF (NWAVE.NE.0) THEN ! WAVE LOAD
	    READ(ITI,*)
	    CALL SHEOFFL (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                  NGP,NCO*NNM,NEX,NEF,NWAVE,IA(LGID))
          ENDIF
          IF (NCURRENT.NE.0)THEN ! CURRENT LOAD
          READ(ITI,*)
          CALL SHEOFFL (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                  NGP,NCO*NNM,NEX,NEF,NCURRENT,IA(LGID))
          ENDIF
          IF (NBREAK.NE.0)THEN ! BREAKING WAVE
          READ(ITI,*)
          CALL SHEOFFL (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                  NGP,NCO*NNM,NEX,NEF,NBREAK,IA(LGID))
          ENDIF
          IF (NWIND.NE.0)THEN ! WIND LOAD
          READ(ITI,*)
          CALL SHEOFFL (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                  NGP,NCO*NNM,NEX,NEF,NWIND,IA(LGID))
          ENDIF
          IF (NWAVECUR.NE.0)THEN ! WAVE + CURRENT
          CALL SHEOFFL (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1    NGP,NCO*NNM,NEX,NEF,NWAVECUR,IA(LGID))
          ENDIF
      ELSEIF (KSPEC.EQ.1) THEN ! REPONSE SPECTRUM
          IF (NWAVE.NE.0)THEN
          READ(ITI,*)
	    CALL SHEOFFLSPEC (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                    NGP,NCO*NNM,NEX,NEF,NWAVE,IA(LGID))
          ENDIF
          IF (NCURRENT.NE.0)THEN ! CURRENT SPECTRUM
          CALL SHEOFFLSPEC (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                    NGP,NCO*NNM,NEX,NEF,NCURRENT,IA(LGID))    
          ENDIF
          IF (NBREAK.NE.0)THEN ! BREAKING WAVE SPECTRUM. IN THIS VERSION IS NOT COMPLETE
          CALL SHEOFFLSPEC (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                    NGP,NCO*NNM,NEX,NEF,NBREAK,IA(LGID))    
          ENDIF
          IF (NWIND.NE.0)THEN ! WIND SPECTRUM
          CALL SHEOFFLSPEC (A(LGP),IA(LGS),A(LCO),IA(LEX),IA(LLM),
     1                    NGP,NCO*NNM,NEX,NEF,NWIND,IA(LGID))
          ENDIF
      ENDIF
...
C	NWLO,NSGLO,NSPLO,NHYDR,NBODY,NTMPLD
      ...
      IF (NLEG.EQ.0.OR.NLOAD_OFF.EQ.1) GOTO 800  
	IF (NWLO.NE.0) THEN
	READ(ITI,*)
...
800   IF (NLEGOFF.NE.0.AND.NLOAD_OFF.EQ.1)THEN  
      !READ (ITI,*) ! READ HEAD
      !READ (ITI,*) ! ELEMENT LOOP
      
      ! INDEX DATA OF OFFSHORE LOADCASE, PLEASE SEE THE DETIAL IN TCL FILES
      READ (ITI,*) NSELF,NWAVE,NCURRENT,NBREAK,NWIND,NWAVECUR  ! INDEX DATA OF OFFSHORE LOADCASE (SOLID ELEMENT)
      
      IF (NSELF.NE.0) THEN
      READ(ITI,*)    
      CALL SOLWTH_OFFSHORE (NWLO,A(LCO),IA(LGS),A(LGP),IA(LLM),
     1	                  A(LMP),IA(LMS),IA(LGID))
      ENDIF
	...
	IF (NWAVE.NE.0) THEN	
	READ(ITI,*)
	CALL SOLOFFL(NWAVE,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NCURRENT.NE.0) THEN	
	READ(ITI,*)
	CALL SOLOFFL(NCURRENT,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NBREAK.NE.0) THEN	
	READ(ITI,*)
	CALL SOLOFFL(NBREAK,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NWIND.NE.0) THEN	
	READ(ITI,*)
	CALL SOLOFFL(NWIND,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NWAVECUR.NE.0) THEN	
	READ(ITI,*)
	CALL SOLOFFL(NWAVECUR,A(LCO),IA(LLM),IA(LGID))
      ENDIF
	
      ELSEIF (KSPEC.EQ.1) THEN ! IRREGULAR WAVE
	IF (NWAVE.NE.0)THEN
	READ(ITI,*)
	CALL SOLOFFLRES(NWAVE,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NCURRENT.NE.0)THEN
	READ(ITI,*)
	CALL SOLOFFLRES(NCURRENT,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NBREAK.NE.0)THEN
	READ(ITI,*)
	CALL SOLOFFLRES(NBREAK,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      IF (NWIND.NE.0)THEN
	READ(ITI,*)
	CALL SOLOFFLRES(NWIND,A(LCO),IA(LLM),IA(LGID))
      ENDIF
      ENDIF
      ENDIF
...
C	===========================================================
      ...
      IF (IOFFL.NE.0) THEN
           IF (KLEG.EQ.NLEG) THEN ! OFFSHORE LOADCASE TOEY 01/2019
           NLOAD_OFF = 1 ! INDEX FOR RUNNING
           IEG       = 1
           GOTO 200
           ELSEIF (KLEG.GT.NLEG.AND.KOFF.LT.NLEGOFF-1) THEN ! OFFSHORE LOADCASE TOEY 01/2019
           KOFF = KOFF + 1
           GOTO 200
           ENDIF
      ENDIF
...
C	  LINEAR STATIC
	...
	1						 KTRAF,KEATH,KCSAL,KOFFL,KSPEC,KDESIGN,KFAST,NSOIL  !KTRAF = TRAFFIC LOAD ,KEATH = EARTHQUAKE ,KCSAL = CONSTRUCTION STAGE ,KOFFL = OFFSHORE LOAD                          !KDESIGN = DESIGN OPTION
...
C	  LINEAR DYNAMIC
	  CASE(5)
	    READ (ITI,*,END=100) NSTEP,LDOPTI,LIMEQ(1),LIMEQ(2),LDEK,KOFFL,KSPEC,KFAST,KOREV,KDESIGN,NSOIL,IMASS,NDYMNASS!,NFREQ
	    IF (KSPEC.EQ.1) ICONTROLSPEC = 1
          IF (NFREQ.EQ.1) ICONTROLSPEC = 1
...
C       FATIGUE ANALYSIS  
        CASE(16)
          READ (ITI,*,END=100) NSTEP,LDOPTI,LIMEQ(1),LIMEQ(2),LDEK,KOFFL,KSPEC,KFATM,KFATJ,KFATL,KFTTD
          IF (KSPEC.EQ.1) ICONTROLSPEC = 1
          NFATIGUEFREQUENCY = 1
          LDESTP = NSTEP  !NUMBER OF LOAD CURVE FOR GROUND MOTION SONGSAK SEP2007
          
        CASE(17)
	    READ (ITI,*,END=100) NSTEP,LDOPTI,LIMEQ(1),LIMEQ(2),LDEK,KOFFL,KSPEC,KFAST,KOREV,KDESIGN,NSOIL,IMASS,NDYMNASS,KFATJ!,NFREQ
          !NSTEP,LDOPTI,LIMEQ(1),LIMEQ(2),LDEK,KOFFL,KSPEC,KFAST,KOREV,KFATM ,KFATJ ,KFATL,KFTTD
          ISOLOP = 5
          KFTTD  = 1 
	    IF (KSPEC.EQ.1) ICONTROLSPEC = 1
...
C     ======================================
      ...
      IF (ISOLOP.EQ.16)THEN
         READ (ITI,*) NROOT,OPRES,STEPSTAT,STEPINCR,STEPEND,FREQ_TIME_ANA
         IVPRT  = NROOT
         ISOLV  = 1
         IEIG   = 3
         NITEM  = 30
...
C     SONGSAK SUPPRESS LANCZOS METHOD ... TO REDUCE MEMORY USAGE IN W ARRAY ... OCT2019 TWEAK SPEED      
      IF(ISOLV.EQ.2) THEN
          WRITE(*,*) '---------------------------------------------'
          WRITE(*,*) '******LANCZOS METHOD IS NOT APPLICABLE*******'
          WRITE(*,*) 'AUTOMATIC SWITCH TO SUBSPACE ITERATION METHOD'
          WRITE(*,*) '---------------------------------------------'
          ISOLV = 1
      ENDIF
...
C	=============================================================
	...
	CALL  DEMREL('DYNL',KNUMT,NUMT,2) 

	DO IUMT = 1,NUMT
	READ(ITI,*) ID,TIME,FACL
	CALL MRELFIL('DYNL',TIME,IUMT,1,1)
	CALL MRELFIL('DYNL',FACL,IUMT,2,1)
	ENDDO
...
C	CALLING NUMT
	CALL  LOCATM('DYNL',KNUMT,NUMT,NC,2)
...
C	TT1 = TIMET(1,I1)
C	TT2 = TIMET(1,I2)
	CALL MRELFIL('DYNL',TT1,I1,1,0)
	CALL MRELFIL('DYNL',TT2,I2,1,0)
...
C	TT1 = TIMET(1,I)
C	TT2 = TIMET(1,I+1)
	CALL MRELFIL('DYNL',TT1,I  ,1,0)
	CALL MRELFIL('DYNL',TT2,I+1,1,0)
...
C	FAC1 = TIMET(2,IT)
C	FAC2 = TIMET(2,IT+1)
	CALL MRELFIL('DYNL',FAC1,IT  ,2,0)
	CALL MRELFIL('DYNL',FAC2,IT+1,2,0)
	VV = FAC1*H1 + FAC2*H2
...
C	READ ELEMENTS TO ASSIGN GEOMETRIC PROP
 300  READ (ITI,*) MEMBA,ISET
      CALL STORAGE_SHELL_THCIKNESS (MEMBA,PROPG(2,ISET))
	INC = 1
	IELEt = IELEt+1
	MEMBB = IELEt
	MEMBA = IELEt
...
C
 ...
 3000 FORMAT (1X,'READ ASSIGNMENT OF GEOMETRIC SETS ',I2,' TO ',I5,
	1        1X,'ELEMENTS')
C
      END
C=====================================================================
...
C     -----------------------------------------------------------------
	...
	IREF  = LLV +NEQ*(ILCN-1)-1
      A(IREF+1:IREF+NEQ) = A(IREF+1:IREF+NEQ) + RLOAD(1:NEQ)
...
C	=====================================================================
C	=====================================================================
      SUBROUTINE LDASEM_NEW (RLOAD,MLOAD,NEFL)
	IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
C     -----------------------------------------------------------------
C	PURPOSE: TO ASSEMBLE THE MATRIX OF LOADS FOR DIFFERENT LOAD CASES
...
C     -----------------------------------------------------------------
	...
	IREF  = LLV +NEQ*(ILCN-1)-1
      ...
      MLR(1:NEFL) = IREF+MLOAD(1:NEFL)
      A(MLR(1:NEFL)) = A(MLR(1:NEFL)) + RLOAD(1:NEFL)
	...
	RETURN
      END
C	=====================================================================
      SUBROUTINE LDASEM_OFFSHORE (RLOAD)
	IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
C     -----------------------------------------------------------------
C	PURPOSE: TO ASSEMBLE THE MATRIX OF LOADS FOR DIFFERENT LOAD CASES
...
220	 DO 250  INO=NODEA,NODEB,INC
 ...
 230      R(IEQ)   = R(IEQ)   + RLOAD
          !NFIEQ    = ((NODEA-1)*6)+1
          IF (VFOMA(1).EQ.1.AND.NFOMA.EQ.1.AND.ISF.EQ.1) RMA(IEQ)   = RMA(IEQ) + (RLOAD/ABS(GRAV))
          IF (VFOMA(2).EQ.1.AND.NFOMA.EQ.1.AND.ISF.EQ.2) RMA(IEQ)   = RMA(IEQ) + (RLOAD/ABS(GRAV))
          IF (VFOMA(3).EQ.1.AND.NFOMA.EQ.1.AND.ISF.EQ.3) RMA(IEQ)   = RMA(IEQ) + (RLOAD/ABS(GRAV))
		GOTO 250
...
C
 1100 FORMAT (1X,'READ & GENERATE NODAL LOADS (GENERAL LOAD CASE);',I5,' NODAL LOADS')
...
C
      END
C	=====================================================================
      SUBROUTINE NODELO_OFFSHORE (ID,MSF,NNL)
	IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
C     -------------------------------------------------------------
C     READS AND GENERATES NODAL LOADS, ADDS CONTRIBUTIONS TO LOAD V
...
C
      RETURN
C
 1100 FORMAT (1X,'READ & GENERATE NODAL LOADS (OFFFSHORE LOAD CASE);',I5,' NODAL LOADS')
 2100 FORMAT (///28X,24HCONCENTRATED NODAL LOADS//
     +        1X,'JOINT',8X,'FORCE-X',6X,'FORCE-Y',6X,'FORCE-Z',5X,
     +        'MOMENT-X',5X,'MOMENT-Y',5X,'MOMENT-Z',5X,'WARPING ',5X,'  TEMP  ',4X,'PORE PRS'/1X,80(1H-))     
 2200 FORMAT (///30X,19HNODAL DISPLACEMENTS//
...
C	-------------------------------------------
	...
	WRITE(ITO,100) IGD,IPT,GXYZ(1:3)
      !WRITE(10,100) IGD,IPT,GXYZ(1:3)
	ELSE
	WRITE(ITO,101)	   IPT,GXYZ(1:3)
      !WRITE(10,101)	   IPT,GXYZ(1:3)
	ENDIF
...
C     --------------------------------------------------------
	...
	WRITE(ITO,100) IGD,IPT,GXYZ(1:3,IGP)
      !WRITE(10,100) IGD,IPT,GXYZ(1:3,IGP)
	ELSE
	WRITE(ITO,101)	   IPT,GXYZ(1:3,IGP)
      !WRITE(10,101)	   IPT,GXYZ(1:3,IGP)
	ENDIF
	ENDDO
...
C	=======================================================================
C	=======================================================================      
      SUBROUTINE LOCALREAD(IGSET)
	IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
C	SUNIL 01/05/09 COMPLETELY MODIFIED
C     ------------------------------------------------------------
C     ASSIGN GEOMETRIC PROPERTY SETS TO EACH ELEMENTS OF THE CURRENT ELEMENT GROUP
C	---------------------------------------------
C     IGSET(NELE)      = GEOMETRIC PROPERTY SET NUMBER
C     ------------------------------------------------------------
      COMMON /ELEM/ NAME(2),ITYPE,ISTYP,NLOPT,MTMOD,NSINC,ITOLEY,
...
C=====================================================================
C=====================================================================      
      SUBROUTINE OUTCONTROL
	IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
      COMMON /INOU/ ITI,ITO,ISO,NDATI,NPLOT,NKFAC,NELEM,
     1              IFPR(10),IFPL(10)
      COMMON /OUTPUTCONTROL/ NLFORCE,NLSTRESS,NSTRESSP,NTORSION,NREATION,NDISPROTA
      
      READ (ITI,*) ! HEAD
      READ (ITI,*) NLFORCE,NLSTRESS,NSTRESSP,NTORSION,NREATION,NDISPROTA
      NLFORCE     = 1
      NLSTRESS    = 1
      NSTRESSP    = 1
      NTORSION    = 1
