C	=======================================================================
C	=======================================================================
      SUBROUTINE FRAMFOC_OFFSHORE_CURRENT (LM,XYZ,IGSET,PROPG,MTSET,PROPM,IGIDM,
	1				    MLOAD,LRPIN,IHSET)
...
C     ========================= READ INPUT DATA AND STORE ON TEMPORARY VARIABLE =========================
      ! NWAVE_INDEX IS USED TO PROVIED THE INFORMATION OF WAVE. 
      NWAVE_INDEX = 0.0D0   ! FOR AUTOCAD 2020
      IF (NWAVE_INDEX.EQ.0) READ (ITI,*) ! ONLY CURRENT LOAD
      !IF (NWAVE_INDEX.NE.0) REWIND (71)  ! WAVE + CURRENT (THE NEW DATA IS PROVIDED IN EXTERNAL FILE)
      
	DO IOFFL = 1,NOFFL
        IF (NWAVE_INDEX.EQ.0) THEN
        ! READING FROM DATIN
     ...
     1              GROWTH,NIRRE,OFFSELECT_X,LWCASE(1),LWCASE(2),LWCASE(5),LWCASE(6),LWCASE(7),ILCN,ILCC !READ ONLY FIRST TIME STEP
        ELSEIF (NWAVE_INDEX.NE.0)THEN
        ! READING FROM EXTERNAL FILE
        READ(71,*) MLE,NFUNCTION,ROUGH,CA,CD,CM,CSA,NWAVESEGOPT,NWAVESEG,NSURFWSEGOPT,NSURFWSEGMT,NDI,DIAM1,NGROWTH,
     1              GROWTH,NIRRE,OFFSELECT_X,LWCASE(1),LWCASE(2),LWCASE(5),LWCASE(6),LWCASE(7),ILCN,ILCC !READ ONLY FIRST TIME STEP    
        ENDIF
        IVAL(1 ,IOFFL)  = MLE
...
C	WATER SURFACE PROFILE MAX
      ...
      YL = WDEPTH + WNU
      IF (LWCASE(1).EQ.0.AND.LWCASE(2).EQ.1) YL = WDEPTH
...
!      ENDIF
      ...
      !   IF(WLEV.GE.YL.AND.WLEV.LE.(YL+DIFFINTERVAL)) THEN
      !       HGR = YL
      !     ENDIF
...
8964  FORMAT(3F12.5,2X,A3,2X,A3)
      ...
      !WRITE(5042,3849)WAVEFORCEXELE1,WAVEFORCEXELE2,WAVEFXI,MLE        ! TOEY 10/21
!      WRITE(*,3849)WAVEFORCEXELE1,WAVEFORCEXELE2,WAVEFXI,MLE           ! TOEY 10/21
!3849  FORMAT('STIFF FORCE',2x, F15.4,2x, F15.4,2x, F15.4, I4)          ! TOEY 10/21
...
!     WAVEFZI = 0.5D0*ZDIFFINTERVAL*( WAVEFORCEZELE1 + WAVEFORCEZELE2 ) 
       ...
       !WRITE(5032,98310)MLE,INDEXTAPPER,WAVEFXI,WAVEFYI,WAVEFZI      ! TOEY 10/21
