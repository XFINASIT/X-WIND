C	=======================================================================
C	=======================================================================
      SUBROUTINE FRAMFOC_OFFSHORE_WIND (LM,XYZ,IGSET,PROPG,MTSET,PROPM,IGIDM,
	1				    MLOAD,LRPIN,IHSET)
...
C     ========================= READ INPUT DATA AND STORE ON TEMPORARY VARIABLE =========================
      READ (ITI,*) 
	DO IOFFL = 1,NOFFL
        READ(ITI,*) MLE,NFUNCTION,ROUGH,CA,CD,CM,CSA,GUST_FACTOR,NWAVESEGOPT,NWAVESEG,NSURFWSEGOPT,NSURFWSEGMT,NDI,DIAM1,NGROWTH,
...
C       ---------------------------------
        ...
        RVAL(3 ,IOFFL) = DIAM2
        RVAL(18,IOFFL) = GUST_FACTOR
        RVAL(19,IOFFL) = NDI
        RVAL(20,IOFFL) = AMARINE(MLE)
...
!      WRITE(5027,11259) -MLE , CHXR , CHYR , CHZR
!11259 FORMAT(I5,4F12.4)
...
!      WRITE(5029,78501)
!78501 FORMAT('ELEMENT  STRIP          ELEVATION            INTERVAL         WAVEFORCE              DIAMETER')           ! TOEY 10/2021
!                                                                                                                        ! TOEY 10/2021
!      WRITE(5031,*)' MLE,   WAVEFORCEXELE1, WAVEFORCEXELE2,    WAVEFXI,         HGM,        CENTROID '                  ! TOEY 10/2021
!      WRITE(5032,*)'  MLE,  INDEXSTRIP,      WAVEFXI,             WAVEFYI,            WAVEFZI'                          ! TOEY 10/2021
!      WRITE(5033,*)'MLE,INDEXTAPPER,RREX1,RREX2,RMEX1,RMEX2,WAVEFXI,WAVEFYI,WAVEFYI,CENTROID'                           ! TOEY 10/2021
!      if( KFTTD .NE.1 ) write(*,*)                                                                                     ! TOEY 10/2021
...
744   FORMAT( 'ELEVA', 2X, 3F10.4 ,2X,3A  )  
      ...
      ENDIF
          ...
          ! =========================================
          ! Incline Structure IN GRAVITY Z DIRECTION
          ! =========================================
             !if (MLE.eq.1) INTERVALTPZ  = 1
             !if (MLE.eq.2) INTERVALTPZ  = 1
             !if (MLE.eq.3) INTERVALTPZ  = 1
             !if (MLE.eq.4) INTERVALTPZ  = 1
             !if (MLE.eq.5) INTERVALTPZ  = 1
             !if (MLE.eq.6) INTERVALTPZ  = 1
             !if (MLE.eq.7) INTERVALTPZ  = 6
             !if (MLE.eq.8) INTERVALTPZ  = 1
             !if (MLE.eq.9) INTERVALTPZ  = 1
             !if (MLE.eq.10) INTERVALTPZ = 1
             !if (MLE.eq.11) INTERVALTPZ = 1
             !if (MLE.eq.12) INTERVALTPZ = 1
             !if (MLE.eq.13) INTERVALTPZ = 1
...
C     GAUSS COORDINATE RELATIVE TO WAVE REFERENCE POINT (IN WAVE DIRECTION SYSTEM)
      ...
      CALL TAPPEROFFSHORETAPIZOIDAL_WIND (DIAMOUTORIGINAL,IVAL(1,IOFFL),XYZ,INDEXTAPPER,INTERVALTPZ,IELETPZ)    
...
!      WRITE(5029,7851) NWAVEELEVA,IGR,HGM,DIFFINTERVAL,WFFC,DIAM,H1R            ! TOEY 10/2021
!7851  FORMAT( I5,2X,I5,7F20.5)                                                  ! TOEY 10/2021
     ...
     1                WKF,CBF,AP,SP,
     1                WFC,YLMIN,
     1                VELO,ACCE,COA,VH1,VGV,YL,XYZ,MLE,IORRE,IORRE,IRWAVE,
     1                ITIME,ILCAS)
...
C          WLEV = 0.00D0
     ...
     2                        CD,CM,DIAM,LCASE,WLEV,WH1,WGV,WH2,WFF) 
            ...
            WFF = 0.5D0*RHOA*DIAM*UZT*UZT
            !WRITE (300,1111) WLEV,DIAM,UZT,WFF
1111        FORMAT (E12.5,E12.5,E12.5,E12.5)
            ELSEIF (NWINDO.EQ.4)THEN ! IEC 614000-1
            ! GID     = UHD
            ! AUTOCAD = UD
            UZT = UH*((WLEV/RHIGH)**ALPHA)
            WFF = 0.5D0*RHOA*DIAM*UZT*UZT
            ENDIF
          ! --------------------------------------------------------------------------------
      
      WFX = VWIND(1)*WFF*CSA*GUSFACTOR ; WFY = VWIND(2)*WFF*CSA*GUSFACTOR ; WFZ = VWIND(3)*WFF*CSA*GUSFACTOR ;
...
!      WRITE(5042,3849)WAVEFORCEXELE1,WAVEFORCEXELE2,WAVEFXI,MLE      ! TOEY 10/2021
!      WRITE(*,3849)WAVEFORCEXELE1,WAVEFORCEXELE2,WAVEFXI,MLE
!3849  FORMAT('STIFF FORCE',2x, F15.4,2x, F15.4,2x, F15.4, I4)        ! TOEY 10/2021
...
!       WRITE(5032,98310)MLE,INDEXTAPPER,WAVEFXI,WAVEFYI,WAVEFZI      ! TOEY 10/2021
!98310  FORMAT(2I7,10F20.5)                                           ! TOEY 10/2021
...
!      ENDDO
      ...
      NWAVEOPT = 1
      CALL TRAPIZOIDAL_REACTION_LCS( ILCAS, IOFFL_CASE , NWAVEOPT , 'WRT' )
...
128	CONTINUE
      ...
      IF(KSPEC.NE.0) THEN
        IF (ISOLOP.EQ.5.AND.ICONTROL.EQ.0) CALL OFFSHFORC(RRV,ILCAS,ITIME,NEQ,'SPEC','WRIT') !STORE VARY LOAD
        IF (ISOLOP.EQ.5.AND.ICONTROL.EQ.1) CALL OFFSHFORC(RRV,ILCAS,ITIME,NEQ,'VARY','WRIT') !STORE VARY LOAD
...
C        CALL OFFSHFORC(RRC,ILCAS,ITIME,NEQ,'CONT','WRIT') !STORE CONSTANT LOAD
      ...
      !IF (ISOLOP.EQ.5) CALL MUTIFORCE (ITIME,ILCAS,ISPEC,NOFFL)
