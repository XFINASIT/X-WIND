C     ==================================================  
      SUBROUTINE FRAMFOC_OFFSHORE_WAVE (LM,XYZ,IGSET,PROPG,MTSET,PROPM,IGIDM,
	1				    MLOAD,LRPIN,IHSET)
...
C     FOR OFFSHORE LOAD -- PRAMIN 
	...
	ALLOCATABLE RRV(:),RRC(:),RVAL(:,:),IVAL(:,:),AMARINE(:)
      ...
      ALLOCATABLE NMLE(:),AOFFM(:),NRAT(:),NILCN(:),NILCC(:),IOFFLTYPE(:)
...
C     ========================= READ INPUT DATA AND STORE ON TEMPORARY VARIABLE =========================
      NLOOP_OF_CU = 1                  ! WAVE LOOP
!      IF (NCURRT.NE.0) NLOOP_OF_CU = 2 ! WAVE AND CURRENT LOOP (REMOVE FOR AUTOCAD 2020)
      
      DO IOF_CUR = 1,NLOOP_OF_CU       ! STARING LOOP OF WAVE + CURRENT 10/2018
      READ (ITI,*) 
      
      IF (IOF_CUR.EQ.1)THEN
	DO IOFFL   = 1,NOFFL
...
C       ---------------------------------
    ...
    ! -------------------------------  FOR ERROR MESSAGE MARINE GROWTH ------------------------------------ 
     ...
     1                        NGROWTH,AMARINE(MLE))
      ...
      ELSEIF (IOF_CUR.EQ.2)THEN ! READ CURRENT
      
      DO KK = 1,NCURRT
      NCURRENT_OPT = 0.
      READ(ITI,*)  MLE,NFUNCTION,ROUGH,CA,CD,CM,CSA,NWAVESEGOPT,NWAVESEG,NSURFWSEGOPT,NSURFWSEGMT,NDI,DIAM1,NGROWTH,
     1              GROWTH,OFFSELECT_X,LWCASE(1),LWCASE(2),LWCASE(5),LWCASE(6),LWCASE(7),ILCN,ILCC !READ ONLY FIRST TIME STEP    
      OPEN(UNIT=71,FILE='Current Data.dat'   ,STATUS='UNKNOWN')
         NDUMMY = 0.
         DO 151 JJ = 1,NOFFL
         IF (IVAL(1 ,JJ).EQ.MLE.AND.IVAL(9 ,JJ).EQ.ILCN)THEN 
             IVAL(3 ,JJ)  = LWCASE(2)
             WRITE (71,150) MLE,NFUNCTION,ROUGH,CA,CD,CM,CSA,NWAVESEGOPT,NWAVESEG,NSURFWSEGOPT,NSURFWSEGMT,NDI,DIAM1,NGROWTH,
     1              GROWTH,OFFSELECT_X,LWCASE(1),NDUMMY,LWCASE(5),LWCASE(6),LWCASE(7),ILCN,ILCC
             NCURRENT_OPT = 1
             EXIT
         ENDIF 
151      CONTINUE
         IF (NCURRENT_OPT.EQ.0)THEN
              WRITE (71,150) MLE,NFUNCTION,ROUGH,CA,CD,CM,CSA,NWAVESEGOPT,NWAVESEG,NSURFWSEGOPT,NSURFWSEGMT,NDI,DIAM1,NGROWTH,
     1              GROWTH,OFFSELECT_X,LWCASE(1),LWCASE(2),LWCASE(5),LWCASE(6),LWCASE(7),ILCN,ILCC 
         ENDIF
         
150          FORMAT (I5,2X,I5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5,2X,I5,2X,I5,2X,I5,2X,I5,2X,I5,2X,E12.5,2X,I5,
     1       2X,E12.5,2X,E12.5,2X,I5,2X,I5,2X,I5,2X,I5,2X,I5,2X,I5,2X,I5)
      ENDDO
...
101    CALL BLOCKDATAOFFSHORE (WVHIGHT,WDEPTH,THIGHT,H1POS,H2POS,IWAVE,ORDER,PERIOD,GRAV,RHOW,RHOA,
 ...
 !     GOTO 117
	...
	IF(IGR.NE.1.AND.IGR.NE.12) GPW(IGR) =  GAUSW(IGR-1,12-2)
      ...
      IF (KFTTD.NE.1) THEN ! NORMAL OFFSHORE ANALYSIS
      ...
      ELSEIF (KFTTD.EQ.1)THEN ! FATIGUE ANALYSIS (LINEAR DYNAMIC ILC = 1)
      CALL OFFSHSTEP(TIME,ITIME,NTIME,'CALL') 
      CALL OFFSHFORC(RRV,INDEXSCAT,ITIME,NEQ,'VARY','READ')
      CALL OFFSHFORC(RRC,INDEXSCAT,ITIME,NEQ,'CONT','READ')    
      ENDIF
      WW(1) = 0.0
      WW(2) = 0.0
      WW(3) = 0.0
...
!      
      ...
      IF(KOFFL.NE.0) THEN
        ...
        CALL OFFSHFORC(RRV,INDEXSCAT,ITIME,NEQ,'VARY','WRIT') !STORE VARY LOAD
        CALL OFFSHFORC(RRC,INDEXSCAT,ITIME,NEQ,'CONT','WRIT') !STORE CONSTANT LOAD
        ENDIF
...
C	======================================================================
      ...
      ALLOCATE(RRV(NEQ))
      ALLOCATE(NMLE(NBOFL),AOFFM(NBOFL),NRAT(NBOFL),NILCN(NBOFL),NILCC(NBOFL),IOFFLTYPE(NBOFL))
C      CALL CLEARA (RRV,NEQ)
      ...
      DO KK = 1,NBOFL
      READ(ITI,*) MLE,OFFTYPE,offselect,NRATIO,ILCN,ILCC  
      NMLE(KK)  = MLE
      IOFFLTYPE(KK) = OFFTYPE
      AOFFM(KK) = offselect
      NRAT(KK)  = NRATIO
      NILCN(KK) = ILCN
      NILCC(KK) = ILCC
...
210   CONTINUE
	...
	ZR = (XYZ(6,MLE)+XYZ(3,MLE))*0.5D0
      ...
      CASE(3)
        RLEV = ZR
      ENDSELECT
      ! CHECKING WAVE CONDITION FOR MODIFY WATER DEPTH
      ...
      GROWTH = 0.0d0
      CALL MARINE_GROWTH_CAL (offselect,RLEV_TOP,GROWTH,DENSITY,IOFFTYPE) 
      
      IF (NRATIO.EQ.1)THEN ! FLOODED
      CALL FRAME_AREA (NMLE(II),GROWTH,AREA_TOP,"FN")
      ELSEIF (NRATIO.EQ.2)THEN ! UNFLOODED
      CALL FRAME_AREA (NMLE(II),GROWTH,AREA_TOP,"UN")    
      ENDIF
      ...
      SELECTCASE(NGRAV)
      CASE(1)
        RLEV_BOTTOM = XYZ(4,MLE)
      CASE(2)
        RLEV_BOTTOM = XYZ(5,MLE)
      CASE(3)
        RLEV_BOTTOM = XYZ(6,MLE)
      ENDSELECT
      
      GROWTH = 0.0d0
      CALL MARINE_GROWTH_CAL (offselect,RLEV_BOTTOM,GROWTH,DENSITY,IOFFTYPE) 
      
      IF (NRATIO.EQ.1)THEN ! FLOODED
      CALL FRAME_AREA (NMLE(II),GROWTH,AREA_BOTTOM,"FN")
      ELSEIF (NRATIO.EQ.2)THEN ! UNFLOODED
      CALL FRAME_AREA (NMLE(II),GROWTH,AREA_BOTTOM,"UN")    
      ENDIF
      
      AREA = (AREA_TOP + AREA_BOTTOM)/2D0
      
      
      OPT = 2D0
      
      IF (RLEV_TOP.LE.WATER_ELEVATION.AND.RLEV_BOTTOM.LE.WATER_ELEVATION) THEN
      DEPTH_ELE = WATER_ELEVATION - RLEV     
      !BUOYANCYF = RHOW*GRAV*DEPTH_ELE*AREA
      BUOYANCYF = RHOW*GRAV*AREA*ELN
      AAL = 0.5D0
      IF (OPT.EQ.2)THEN
      AAL = 0.0D0
      BBL = 1.0D0-AAL
      BUOYANCYF = RHOW*GRAV*AREA
      ENDIF
      ELSEIF (RLEV_TOP.LE.WATER_ELEVATION.OR.RLEV_BOTTOM.LE.WATER_ELEVATION)THEN
          IF (RLEV_TOP.GE.RLEV_BOTTOM) THEN
              ! CALCULATE NEW COORDINATE 
              !ELN_BUO = WATER_ELEVATION - RLEV_BOTTOM 
              
              SELECTCASE(NGRAV)
              CASE(1)
              X_T = XYZ(4,MLE) - XYZ(1,MLE)
              Y_T = XYZ(5,MLE) - XYZ(2,MLE)
              Z_T = XYZ(6,MLE) - XYZ(3,MLE)
              ANEW_X = WATER_ELEVATION
              ANEW_Y = (((WATER_ELEVATION-XYZ(1,MLE))/X_T)*Y_T) + XYZ(2,MLE)
              ANEW_Z = (((WATER_ELEVATION-XYZ(1,MLE))/X_T)*Z_T) + XYZ(3,MLE)
              CASE(2)
              X_T = XYZ(4,MLE) - XYZ(1,MLE)
              Y_T = XYZ(5,MLE) - XYZ(2,MLE)
              Z_T = XYZ(6,MLE) - XYZ(3,MLE)
              ANEW_X = (((WATER_ELEVATION-XYZ(2,MLE))/Y_T)*X_T) + XYZ(1,MLE)
              ANEW_Y = WATER_ELEVATION
              ANEW_Z = (((WATER_ELEVATION-XYZ(2,MLE))/Y_T)*Z_T) + XYZ(3,MLE)
              CASE(3)
              X_T = XYZ(4,MLE) - XYZ(1,MLE)
              Y_T = XYZ(5,MLE) - XYZ(2,MLE)
              Z_T = XYZ(6,MLE) - XYZ(3,MLE)
              ANEW_X = (((WATER_ELEVATION-XYZ(3,MLE))/Z_T)*X_T) + XYZ(1,MLE)
              ANEW_Y = (((WATER_ELEVATION-XYZ(3,MLE))/Z_T)*Y_T) + XYZ(2,MLE)
              ANEW_Z = WATER_ELEVATION
              ENDSELECT
              ELN_BUO = SQRT((ANEW_X-XYZ(4,MLE))**2D0 + (ANEW_Y-XYZ(5,MLE))**2D0 + (ANEW_Z-XYZ(6,MLE))**2D0 )
          ...
          ELSEIF (RLEV_TOP.LT.RLEV_BOTTOM) THEN
              !ELN_BUO = WATER_ELEVATION - RLEV_TOP
              ...
              ANEW_Z = WATER_ELEVATION
              ENDSELECT
              ELN_BUO = SQRT((ANEW_X-XYZ(1,MLE))**2D0 + (ANEW_Y-XYZ(2,MLE))**2D0 + (ANEW_Z-XYZ(3,MLE))**2D0 )
      ...
      IF (OPT.EQ.1)THEN        
      BUOYANCYF = RHOW*GRAV*AREA*ELN_BUO
      AAL = ABS(ELN_BUO/ELN)
      ENDIF
      
      ELSEIF (RLEV_TOP.GT.WATER_ELEVATION.AND.RLEV_BOTTOM.GT.WATER_ELEVATION) THEN
      BUOYANCYF = 0.0D0
      AAL = 0.5D0
      ENDIF
      ...
      ! FORCE_B = 0.
      ! IF (NGRAV.EQ.1)THEN
      ! FORCE_A(1) = ABS(RHOW*GRAV*AREA*XYZ(3,MLE))
      ! FORCE_B(1) = ABS(RHOW*GRAV*AREA*XYZ(6,MLE))
      ! ELSEIF (NGRAV.EQ.2)THEN
      ! FORCE_A(2) = ABS(RHOW*GRAV*AREA*XYZ(3,MLE))
      ! FORCE_B(2) = ABS(RHOW*GRAV*AREA*XYZ(6,MLE))    
      ! ELSEIF (NGRAV.EQ.3)THEN
      ! FORCE_A(3) = ABS(RHOW*GRAV*AREA*XYZ(3,MLE))
      ! FORCE_B(3) = ABS(RHOW*GRAV*AREA*XYZ(6,MLE))
      ! ENDIF
...
C	-----------------------------------------------------
	...
	IND = 1
      ELSEIF (OPT.EQ.2)THEN
      ! DISTRIBUTED PARTIAL GLOBAL PROJECT LOAD
      RP   = WW(2*I-1)
	RPB  = WW(2*I-0)
	DISA = AAL*ELN
	DISB = BBL*ELN
...
130   CONTINUE
      ...
      DEALLOCATE(RRV)
      DEALLOCATE (NMLE,AOFFM,NRAT,NILCN,NILCC,IOFFLTYPE)
120   CONTINUE
