...
SUBROUTINE GetInputFileName(InputFile,UseDWM,ErrStat,ErrMsg)
   ...
   InputFile = 'primary1.fst'  ! initialize to empty string to make sure it's input from the command line
...
SUBROUTINE FAST_ReadPrimaryFile( InputFile, p, m_FAST, OverrideAbortErrLev, ErrStat, ErrMsg )
   ...
   CALL ReadVar( UnIn, InputFile, p%UJacSclFact, "UJacSclFact", "Scaling factor used in Jacobians (-)", ErrStat2, ErrMsg2, UnEc)
    ...
    ! NumPara - The number of Parameters: Toey
   CALL ReadVar( UnIn, InputFile, p%NumPara, "NumPara", "The number of Parameters", ErrStat2, ErrMsg2, UnEc)
       CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
      if ( ErrStat >= AbortErrLev ) then
         call cleanup()
         RETURN        
      end if   
    FILENUMBERINTER = p%NumPara   
