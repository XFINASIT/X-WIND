...
module BEMTUnCoupled
   ...
   public :: ShenTipCorrectionFactor ! shen's Correction Factor
   public :: ShenTaTipCorrectionFactor ! shen's Correction Factor
...
!..................................................................................................................................
subroutine Transform_ClCd_to_CxCy( phi, useAIDrag, useTIDrag, Cl, Cd, Cx, Cy, TSR, B, useShenTipLoss, tipLossConst)
   ...
   real(ReKi),             intent(in   ) :: TSR
   integer,                intent(in   ) :: B              !< number of blades [p%numBlades]
   logical,                intent(in   ) :: useShenTipLoss     !< tip-loss flag [p%useTipLoss]
   real(ReKi),             intent(in   ) :: tipLossConst   !< tip loss constant [p%tipLossConst]
   real(ReKi),             intent(inout) :: Cx
   real(ReKi),             intent(inout) :: Cy
   ...
   F1 = ShenTipCorrectionFactor(sphi, useShenTipLoss, tipLossConst, TSR, B)
   F2 = ShenTaTipCorrectionFactor(sphi, useShenTipLoss, tipLossConst, TSR, B)
...
real(ReKi) function BEMTU_InductionWithResidual(p, u, i, j, phi, AFInfo, IsValidSolution, ErrStat, ErrMsg, a, ap) result (ResidualVal)
   ...
   ! FIX ME: Note that the Re used here is computed assuming axInduction and tanInduction are 0. Is that a problem for 2D Re interpolation on airfoils? or should update solve method to take this into account?
      ...
      call Transform_ClCd_to_CxCy( phi, p%useAIDrag, p%useTIDrag, AFI_interp%Cl, AFI_interp%Cd, Cx, Cy, u%TSR, p%numBlades, p%useShenTipLoss, p%tipLossConst(i,j))  
      
      
         ! Determine axInduction, tanInduction for the current Cl, Cd, phi
      if (p%useShenTipLoss) then
          call ShenInductionFactors( u%rlocal(i,j), p%chord(i,j), phi, Cx, Cy, p%numBlades, &
          ...
          call BuhlInductionFactors( u%rlocal(i,j), p%chord(i,j), phi, Cx, Cy, p%numBlades, &
                                  u%Vx(i,j), u%Vy(i,j), p%useTanInd, p%useHubLoss, p%useTipLoss,  p%hubLossConst(i,j), p%tipLossConst(i,j), &
                                  ResidualVal, axInduction, tanInduction, IsValidSolution)
      end if
...
!-----------------------------------------------------------------------------------------
!> This subroutine computes the induction factors (a) and (ap) along with the residual (fzero) Using Shen's correction
subroutine ShenInductionFactors(r, chord, phi, cn, ct, B, Vx, Vy, wakerotation, useHubLoss, useTipLoss, hubLossConst, tipLossConst, &
   ...
   real(ReKi) :: F1 ! shen's tip losss correction factor
   real(ReKi) :: F2 ! shen's tip loss tangential direction correction factor
   ...
   if (momentumRegion) then  ! momentum/empirical
      ...
      if (k <= 1.0_ReKi/2.0_ReKi) then  ! momentum state for a < 1/3
         
         if ( EqualRealNos(k,-1.0_ReKi) ) then
            a = -sign(InductionLimit, 1.0_ReKi+k)
         else
            a = (2.0_ReKi*k+1.0_ReKi-sqrt(4.0_ReKi*k*(1.0_ReKi-F)+1.0_ReKi))/2.0_ReKi/(k+F)
         end if
      ...
      else  ! Shen's correction for 1/3<a<1

         g1 = 2.0_ReKi*k+1.0_ReKi-2.0_ReKi/3.0_ReKi*F
         g2 = 4.0_ReKi*k-20.0_ReKi/9.0_ReKi*k*F-4.0_ReKi/3.0_ReKi*F+1.0_ReKi+4.0_ReKi/9.0_ReKi*F*F
         g3 = 2.0_ReKi*k

         if (abs(g3) < 1e-6_ReKi) then  ! avoid singularity
            a = 1.0_ReKi - 0.5_ReKi/sqrt(g2)
            print *, "singularity occurs!", a
         else
      ...
      if ( EqualRealNos(k,1.0_ReKi) ) then
         ...
         a = (2.0_ReKi*k-1.0_ReKi-sqrt(4.0_ReKi*k*(F-1.0_ReKi)+1.0_ReKi))/2.0_ReKi/(k-F)
         print *, "brake state"
      end if
   ...
   if ( EqualRealNos(cphi,0.0_ReKi) ) then
      ...
      if ( EqualRealNos(kp,1.0_ReKi) ) then
         ...
         ap = 1.0_ReKi/((1.0_ReKi-a*F)/(1.0_ReKi-a)/kp-1.0_ReKi)
      end if
   ...
   if (momentumRegion) then  ! momentum/empirical
      if ( EqualRealNos(a, 1.0_ReKi) ) then
         fzero = - cphi/lambda_r*(1.0_ReKi-kp)
      else       
         fzero = sphi/(1.0_ReKi-a) - cphi/lambda_r*(1-kp)
      end if
      
   else  ! propeller brake region
      fzero = sphi*(1.0_ReKi-k) - cphi/lambda_r*(1.0_ReKi-kp)
   end if

end subroutine ShenInductionFactors
!-----------------------------------------------------------------------------------------
!> This subroutine computes the induction factors (a) and (ap) along with the residual (fzero) Using Buhl's correction
subroutine BuhlInductionFactors(r, chord, phi, cn, ct, B, Vx, Vy, wakerotation, useHubLoss, useTipLoss, hubLossConst, tipLossConst, &
                              fzero, a, ap, IsValidSolution)
...
end subroutine BuhlInductionFactors
!-----------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------
!> This function computes Shen's TipLoss correction Factor
real(reKi) function ShenTipCorrectionFactor(sphi, useShenTipLoss, tipLossConst, TSR, B) result(Fts)

   real(ReKi), intent(in) :: sphi           !< sine of local inflow angle, sin(phi)
   logical,    intent(in) :: useShenTipLoss     !< tip-loss flag [p%useTipLoss]
   real(ReKi), intent(in) :: tipLossConst   !< tip loss constant [p%tipLossConst]
   real(ReKi), intent(in) :: TSR            !< tip speed ratio 
   integer,    intent(in) :: B              !< Number of blades

   real(ReKi) :: Ftip, factortip, g, x1, x2, x3, tip, n, dcdr


   Ftip = 1.0_ReKi     ! default tip loss value 
   
   if (.not. EqualRealNos(sphi,0.0_ReKi)) then
      if ( useShenTipLoss ) then

         ! From Zhong's paper : tip shape factor (dcdr, n)
         dcdr = 0.0_ReKi 
         n = 1.0_ReKi + 0.5_ReKi*dcdr

         x1 = B**(n-1.0_Reki)
         x2 = 2.0_ReKi**(n-1.0_Reki)
         x3 = tipLossConst**n
         tip = x3*x2/x1
         g=exp(-0.125_ReKi*(B*TSR-21.0_ReKi)/(1.0_ReKi-2.0_ReKi*dcdr))+0.1_ReKi
         factortip = g*tip/abs(sphi)

         Ftip = TwoByPi*acos(exp(min(1.0_ReKi,-factortip)))
      end if
   end if

   Fts = Ftip
   
end function ShenTipCorrectionFactor
!-----------------------------------------------------------------------------------------
!> This function computes Shen's TipLoss correction Factor
real(reKi) function ShenTaTipCorrectionFactor(sphi, useShenTipLoss, tipLossConst, TSR, B) result(Fts)
   ...
   if (.not. EqualRealNos(sphi,0.0_ReKi)) then
      if ( useShenTipLoss ) then
         ...
         x3 = tipLossConst**n
         tip = x3*x2/x1
         g=exp(-0.1652_ReKi*(B*TSR-17.732_ReKi)/(1.0_ReKi-2.0_ReKi*dcdr))+0.1_ReKi ! c1 = -0.1652, c2 = 17.732
         factortip = g*tip/abs(sphi)
...
end function ShenTaTipCorrectionFactor
!-----------------------------------------------------------------------------------------
