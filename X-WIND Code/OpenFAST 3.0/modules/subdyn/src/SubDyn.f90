...
ALLOCATE ( p%MoutLst_XSEA(p%NMEMBERS), STAT = ErrStat2 )     !this list contains different arrays for each of its elements
   
DO I = 1,p%NMEMBERS
p%MOutLst_XSEA(I)%MemberID = I 
p%MOutLst_XSEA(I)%NOutCnt = 2
CALL AllocAry( p%MOutLst_XSEA(I)%NodeCnt, p%MOutLst_XSEA(I)%NOutCnt, 'NodeCnt', ErrStat2, ErrMsg2); if(Failed()) return
p%MOutLst_XSEA(I)%NodeCnt(1) = 1
p%MOutLst_XSEA(I)%NodeCnt(2) = 2
ENDDO
