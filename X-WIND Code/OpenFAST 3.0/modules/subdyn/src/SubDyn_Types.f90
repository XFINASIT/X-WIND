...
! =========  SD_ParameterType  =======
  TYPE, PUBLIC :: SD_ParameterType
    ...
    REAL(ReKi) , DIMENSION(:,:), ALLOCATABLE  :: TIreact_XSEA      !< Matrix to calculate single point reaction at base of structure [-]
    ...
    TYPE(MeshAuxDataType) , DIMENSION(:), ALLOCATABLE  :: MoutLst_XSEA      !< List of user requested members and nodes [-]
    ...
    TYPE(MeshAuxDataType) , DIMENSION(:), ALLOCATABLE  :: MoutLst3_XSEA      !< List of all member joint nodes and elements for output [-]
...
ENDIF
IF (ALLOCATED(SrcParamData%TIreact_XSEA)) THEN
  i1_l = LBOUND(SrcParamData%TIreact_XSEA,1)
  i1_u = UBOUND(SrcParamData%TIreact_XSEA,1)
  i2_l = LBOUND(SrcParamData%TIreact_XSEA,2)
  i2_u = UBOUND(SrcParamData%TIreact_XSEA,2)
  IF (.NOT. ALLOCATED(DstParamData%TIreact_XSEA)) THEN 
    ALLOCATE(DstParamData%TIreact_XSEA(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%TIreact_XSEA.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DstParamData%TIreact_XSEA = SrcParamData%TIreact_XSEA
ENDIF
...
IF (ALLOCATED(SrcParamData%MoutLst)) THEN
  ...
  END IF
    ...
    ENDDO
ENDIF
IF (ALLOCATED(SrcParamData%MoutLst_XSEA)) THEN
  i1_l = LBOUND(SrcParamData%MoutLst_XSEA,1)
  i1_u = UBOUND(SrcParamData%MoutLst_XSEA,1)
  IF (.NOT. ALLOCATED(DstParamData%MoutLst_XSEA)) THEN 
    ALLOCATE(DstParamData%MoutLst_XSEA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%MoutLst_XSEA.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DO i1 = LBOUND(SrcParamData%MoutLst_XSEA,1), UBOUND(SrcParamData%MoutLst_XSEA,1)
      CALL SD_Copymeshauxdatatype( SrcParamData%MoutLst_XSEA(i1), DstParamData%MoutLst_XSEA(i1), CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
    ENDDO
ENDIF
...
IF (ALLOCATED(SrcParamData%MoutLst3)) THEN
  ...
  END IF
    ...
    ENDDO
ENDIF
IF (ALLOCATED(SrcParamData%MoutLst3_XSEA)) THEN
  i1_l = LBOUND(SrcParamData%MoutLst3_XSEA,1)
  i1_u = UBOUND(SrcParamData%MoutLst3_XSEA,1)
  IF (.NOT. ALLOCATED(DstParamData%MoutLst3_XSEA)) THEN 
    ALLOCATE(DstParamData%MoutLst3_XSEA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
      CALL SetErrStat(ErrID_Fatal, 'Error allocating DstParamData%MoutLst3_XSEA.', ErrStat, ErrMsg,RoutineName)
      RETURN
    END IF
  END IF
    DO i1 = LBOUND(SrcParamData%MoutLst3_XSEA,1), UBOUND(SrcParamData%MoutLst3_XSEA,1)
      CALL SD_Copymeshauxdatatype( SrcParamData%MoutLst3_XSEA(i1), DstParamData%MoutLst3_XSEA(i1), CtrlCode, ErrStat2, ErrMsg2 )
         CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg,RoutineName)
         IF (ErrStat>=AbortErrLev) RETURN
    ENDDO
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%TIreact_XSEA)) THEN
  DEALLOCATE(ParamData%TIreact_XSEA)
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%MoutLst_XSEA)) THEN
DO i1 = LBOUND(ParamData%MoutLst_XSEA,1), UBOUND(ParamData%MoutLst_XSEA,1)
  CALL SD_Destroymeshauxdatatype( ParamData%MoutLst_XSEA(i1), ErrStat, ErrMsg )
ENDDO
  DEALLOCATE(ParamData%MoutLst_XSEA)
ENDIF
...
ENDIF
IF (ALLOCATED(ParamData%MoutLst3_XSEA)) THEN
DO i1 = LBOUND(ParamData%MoutLst3_XSEA,1), UBOUND(ParamData%MoutLst3_XSEA,1)
  CALL SD_Destroymeshauxdatatype( ParamData%MoutLst3_XSEA(i1), ErrStat, ErrMsg )
ENDDO
  DEALLOCATE(ParamData%MoutLst3_XSEA)
ENDIF
...
ENDIF
 ...
 ! buffers to store subtypes, if any
  ...
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! TIreact_XSEA allocated yes/no
  IF ( ALLOCATED(InData%TIreact_XSEA) ) THEN
    Int_BufSz   = Int_BufSz   + 2*2  ! TIreact_XSEA upper/lower bounds for each dimension
      Re_BufSz   = Re_BufSz   + SIZE(InData%TIreact_XSEA)  ! TIreact_XSEA
  END IF
  ...
  IF ( ALLOCATED(InData%MoutLst) ) THEN
    ...
    END DO
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! MoutLst_XSEA allocated yes/no
  IF ( ALLOCATED(InData%MoutLst_XSEA) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! MoutLst_XSEA upper/lower bounds for each dimension
    DO i1 = LBOUND(InData%MoutLst_XSEA,1), UBOUND(InData%MoutLst_XSEA,1)
      Int_BufSz   = Int_BufSz + 3  ! MoutLst_XSEA: size of buffers for each call to pack subtype
      CALL SD_Packmeshauxdatatype( Re_Buf, Db_Buf, Int_Buf, InData%MoutLst_XSEA(i1), ErrStat2, ErrMsg2, .TRUE. ) ! MoutLst_XSEA 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN

      IF(ALLOCATED(Re_Buf)) THEN ! MoutLst_XSEA
         Re_BufSz  = Re_BufSz  + SIZE( Re_Buf  )
         DEALLOCATE(Re_Buf)
      END IF
      IF(ALLOCATED(Db_Buf)) THEN ! MoutLst_XSEA
         Db_BufSz  = Db_BufSz  + SIZE( Db_Buf  )
         DEALLOCATE(Db_Buf)
      END IF
      IF(ALLOCATED(Int_Buf)) THEN ! MoutLst_XSEA
         Int_BufSz = Int_BufSz + SIZE( Int_Buf )
         DEALLOCATE(Int_Buf)
      END IF
    END DO
  ...
  IF ( ALLOCATED(InData%MoutLst3) ) THEN
    ...
    END DO
  END IF
  Int_BufSz   = Int_BufSz   + 1     ! MoutLst3_XSEA allocated yes/no
  IF ( ALLOCATED(InData%MoutLst3_XSEA) ) THEN
    Int_BufSz   = Int_BufSz   + 2*1  ! MoutLst3_XSEA upper/lower bounds for each dimension
    DO i1 = LBOUND(InData%MoutLst3_XSEA,1), UBOUND(InData%MoutLst3_XSEA,1)
      Int_BufSz   = Int_BufSz + 3  ! MoutLst3_XSEA: size of buffers for each call to pack subtype
      CALL SD_Packmeshauxdatatype( Re_Buf, Db_Buf, Int_Buf, InData%MoutLst3_XSEA(i1), ErrStat2, ErrMsg2, .TRUE. ) ! MoutLst3_XSEA 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN

      IF(ALLOCATED(Re_Buf)) THEN ! MoutLst3_XSEA
         Re_BufSz  = Re_BufSz  + SIZE( Re_Buf  )
         DEALLOCATE(Re_Buf)
      END IF
      IF(ALLOCATED(Db_Buf)) THEN ! MoutLst3_XSEA
         Db_BufSz  = Db_BufSz  + SIZE( Db_Buf  )
         DEALLOCATE(Db_Buf)
      END IF
      IF(ALLOCATED(Int_Buf)) THEN ! MoutLst3_XSEA
         Int_BufSz = Int_BufSz + SIZE( Int_Buf )
         DEALLOCATE(Int_Buf)
      END IF
    END DO
  ...
  ELSE
    ...
    Int_Xferred = Int_Xferred + 2
      ...
      END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%TIreact_XSEA) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TIreact_XSEA,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TIreact_XSEA,1)
    Int_Xferred = Int_Xferred + 2
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%TIreact_XSEA,2)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%TIreact_XSEA,2)
    Int_Xferred = Int_Xferred + 2

      DO i2 = LBOUND(InData%TIreact_XSEA,2), UBOUND(InData%TIreact_XSEA,2)
        DO i1 = LBOUND(InData%TIreact_XSEA,1), UBOUND(InData%TIreact_XSEA,1)
          ReKiBuf(Re_Xferred) = InData%TIreact_XSEA(i1,i2)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
  ...
  ELSE
    ...
    END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%MoutLst_XSEA) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%MoutLst_XSEA,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%MoutLst_XSEA,1)
    Int_Xferred = Int_Xferred + 2

    DO i1 = LBOUND(InData%MoutLst_XSEA,1), UBOUND(InData%MoutLst_XSEA,1)
      CALL SD_Packmeshauxdatatype( Re_Buf, Db_Buf, Int_Buf, InData%MoutLst_XSEA(i1), ErrStat2, ErrMsg2, OnlySize ) ! MoutLst_XSEA 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
  ...
  ELSE
    ...
    END DO
  END IF
  IF ( .NOT. ALLOCATED(InData%MoutLst3_XSEA) ) THEN
    IntKiBuf( Int_Xferred ) = 0
    Int_Xferred = Int_Xferred + 1
  ELSE
    IntKiBuf( Int_Xferred ) = 1
    Int_Xferred = Int_Xferred + 1
    IntKiBuf( Int_Xferred    ) = LBOUND(InData%MoutLst3_XSEA,1)
    IntKiBuf( Int_Xferred + 1) = UBOUND(InData%MoutLst3_XSEA,1)
    Int_Xferred = Int_Xferred + 2

    DO i1 = LBOUND(InData%MoutLst3_XSEA,1), UBOUND(InData%MoutLst3_XSEA,1)
      CALL SD_Packmeshauxdatatype( Re_Buf, Db_Buf, Int_Buf, InData%MoutLst3_XSEA(i1), ErrStat2, ErrMsg2, OnlySize ) ! MoutLst3_XSEA 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
 ...
 ! buffers to store meshes, if any
  ...
  ELSE
    ...
    END IF
      ...
      END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! TIreact_XSEA not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    ...
    i2_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%TIreact_XSEA)) DEALLOCATE(OutData%TIreact_XSEA)
    ALLOCATE(OutData%TIreact_XSEA(i1_l:i1_u,i2_l:i2_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%TIreact_XSEA.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
      DO i2 = LBOUND(OutData%TIreact_XSEA,2), UBOUND(OutData%TIreact_XSEA,2)
        DO i1 = LBOUND(OutData%TIreact_XSEA,1), UBOUND(OutData%TIreact_XSEA,1)
          OutData%TIreact_XSEA(i1,i2) = ReKiBuf(Re_Xferred)
          Re_Xferred = Re_Xferred + 1
        END DO
      END DO
  END IF
  ...
  ELSE
    ...
    END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! MoutLst_XSEA not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%MoutLst_XSEA)) DEALLOCATE(OutData%MoutLst_XSEA)
    ALLOCATE(OutData%MoutLst_XSEA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%MoutLst_XSEA.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    DO i1 = LBOUND(OutData%MoutLst_XSEA,1), UBOUND(OutData%MoutLst_XSEA,1)
      Buf_size=IntKiBuf( Int_Xferred )
      Int_Xferred = Int_Xferred + 1
      IF(Buf_size > 0) THEN
        ALLOCATE(Re_Buf(Buf_size),STAT=ErrStat2)
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL SD_Unpackmeshauxdatatype( Re_Buf, Db_Buf, Int_Buf, OutData%MoutLst_XSEA(i1), ErrStat2, ErrMsg2 ) ! MoutLst_XSEA 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
  ...
  ELSE
    ...
    END DO
  END IF
  IF ( IntKiBuf( Int_Xferred ) == 0 ) THEN  ! MoutLst3_XSEA not allocated
    Int_Xferred = Int_Xferred + 1
  ELSE
    Int_Xferred = Int_Xferred + 1
    i1_l = IntKiBuf( Int_Xferred    )
    i1_u = IntKiBuf( Int_Xferred + 1)
    Int_Xferred = Int_Xferred + 2
    IF (ALLOCATED(OutData%MoutLst3_XSEA)) DEALLOCATE(OutData%MoutLst3_XSEA)
    ALLOCATE(OutData%MoutLst3_XSEA(i1_l:i1_u),STAT=ErrStat2)
    IF (ErrStat2 /= 0) THEN 
       CALL SetErrStat(ErrID_Fatal, 'Error allocating OutData%MoutLst3_XSEA.', ErrStat, ErrMsg,RoutineName)
       RETURN
    END IF
    DO i1 = LBOUND(OutData%MoutLst3_XSEA,1), UBOUND(OutData%MoutLst3_XSEA,1)
      Buf_size=IntKiBuf( Int_Xferred )
      Int_Xferred = Int_Xferred + 1
      IF(Buf_size > 0) THEN
        ALLOCATE(Re_Buf(Buf_size),STAT=ErrStat2)
      ...
      IF(Buf_size > 0) THEN
        ...
        Int_Xferred = Int_Xferred + Buf_size
      END IF
      CALL SD_Unpackmeshauxdatatype( Re_Buf, Db_Buf, Int_Buf, OutData%MoutLst3_XSEA(i1), ErrStat2, ErrMsg2 ) ! MoutLst3_XSEA 
        CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
        IF (ErrStat >= AbortErrLev) RETURN
