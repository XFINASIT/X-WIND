...
SUBROUTINE SDOut_Init( Init, y,  p, misc, InitOut, WtrDpth, ErrStat, ErrMsg )
   ...
   type(MeshAuxDataType), pointer :: pLst_XSEA
   real(ReKi), allocatable :: T_TIreact(:,:) ! Transpose of TIreact, temporary
   real(ReKi), allocatable :: T_TIreact_XSEA(:,:) ! Transpose of TIreact, temporary
   ErrStat = 0      
   ErrMsg=""
   ...
   ENDDO  !I, NMOutputs
      ...
      DO I=1,p%NMEMBERS!,p%NMOutputs
      pLst_XSEA => p%MOutLst_XSEA(I) ! Alias to shorten notations
      CALL AllocAry(pLst_XSEA%NodeIDs,    pLst_XSEA%NoutCnt   , 'MOutLst_XSEA(I)%NodeIDs', ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry(pLst_XSEA%ElmIDs,     pLst_XSEA%NoutCnt, 2, 'MOutLst_XSEA(I)%ElmIDs' , ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry(pLst_XSEA%ElmNds,     pLst_XSEA%NoutCnt, 2, 'MOutLst_XSEA(I)%ElmNds' , ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry(pLst_XSEA%Me, 12, 12, pLst_XSEA%NoutCnt, 2, 'MOutLst_XSEA(I)%Me'     , ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry(pLst_XSEA%Ke, 12, 12, pLst_XSEA%NoutCnt, 2, 'MOutLst_XSEA(I)%Ke'     , ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry(pLst_XSEA%Fg,     12, pLst_XSEA%NoutCnt, 2, 'MOutLst_XSEA(I)%Fg'     , ErrStat2, ErrMsg2); if(Failed()) return
      ! NOTE: len(MemberNodes) >2 if nDiv>1
      iMember = FINDLOCI(Init%Members(:,1), pLst_XSEA%MemberID) ! Reindexing from MemberID to 1:nMembers
      pLst_XSEA%NodeIDs(1:pLst_XSEA%NoutCnt)=Init%MemberNodes(iMember, pLst_XSEA%NodeCnt)  ! We are storing the actual node numbers corresponding to what the user ordinal number is requesting
      pLst_XSEA%ElmIDs=0  !Initialize to 0
      pLst_XSEA%ElmNds=0  !Initialize to 0

      DO J=1,pLst_XSEA%NoutCnt ! loop on requested nodes for that member
         iNode        = pLst_XSEA%NodeIDs(J)           ! Index of requested node in node list
         nElemPerNode = Init%NodesConnE(iNode, 1) ! Number of elements connecting to the j-th node
         ! Finding 1 or max 2 elements that belong to the member and connect to the node
         K2=0 ! Counter so that max 2 elements are included: NOTE: I belive more than 2 should be an error
         DO iiElem = 1, nElemPerNode
            ...
            IF (ThisElementIsAlongMember(iElem, iNode, iMember)) THEN
               IF (K2 == 2) EXIT ! we found both elements already, error...
               K2=K2+1
               call ConfigOutputNode_MKF_ID(pLst_XSEA, iElem, iiNode=J, iStore=K2, NodeID2=iNode)
            END IF    
         ENDDO  ! iiElem, nElemPerNode
       ENDDO !J, Noutcnt
      ENDDO  !I, NMOutputs
   ...
   ENDIF
    ...
    ! MOutLst3_XSEA: nodal output info by members, for the members involved in reaction
      ALLOCATE(p%MOutLst3_XSEA(p%nNodes_C), STAT = ErrStat2); ErrMsg2 = 'Error allocating p%MOutLst3_XSEA array in SDOut_Init'; if(Failed()) return

      DO I=1,p%nNodes_C  !For all constrained node
         pLst_XSEA => p%MOutLst3_XSEA(I)
         iNode        = p%Nodes_C(I,1)           ! Note: Nodes_C has been reindexed
         nElemPerNode = Init%NodesConnE(iNode,1) ! Number of elements connecting to the joint
         CALL AllocAry(pLst_XSEA%ElmIDs,      1, nElemPerNode, ' p%MOutLst3_XSEA(I)%ElmIds', ErrStat2, ErrMsg2); if(Failed()) return
         CALL AllocAry(pLst_XSEA%ElmNds,      1, nElemPerNode, ' p%MOutLst3_XSEA(I)%ElmNds', ErrStat2, ErrMsg2); if(Failed()) return
         CALL AllocAry(pLst_XSEA%Me, 12, 12 , 1, nElemPerNode, ' p%MOutLst3_XSEA(I)%Me'    , ErrStat2, ErrMsg2); if(Failed()) return
         CALL AllocAry(pLst_XSEA%Ke, 12, 12 , 1, nElemPerNode, ' p%MOutLst3_XSEA(I)%Ke'    , ErrStat2, ErrMsg2); if(Failed()) return
         CALL AllocAry(pLst_XSEA%Fg,     12 , 1, nElemPerNode, ' p%MOutLst3_XSEA(I)%Fg'    , ErrStat2, ErrMsg2); if(Failed()) return
         DO iiElem = 1, nElemPerNode
            iElem = Init%NodesConnE(iNode, iiElem+1) ! iiElem-th Element Number in the set of elements attached to the selected node 
            call ConfigOutputNode_MKF_ID(pLst_XSEA, iElem, iiNode=1, iStore=iiElem, NodeID2=iNode) 
         ENDDO
         p%MOutLst3_XSEA(I)%NOUTCNT = iNode
      ENDDO
      ! Compute p%TIreact, rigid transf. matrix from reaction DOFs to base structure point (0,0,-WD)
      CALL AllocAry(p%TIreact_XSEA, 6, p%nDOFC__, 'TIReact_XSEA  ', ErrStat2, ErrMsg2); if(Failed()) return
      CALL AllocAry(T_TIreact_XSEA, p%nDOFC__, 6, 'TIReact_T_XSEA', ErrStat2, ErrMsg2); if(Failed()) return
      call RigidTrnsf(Init, p, (/0.0_Reki, 0.0_ReKi, -WtrDpth /), p%IDC__, p%nDOFC__, T_TIreact_XSEA, ErrStat2, ErrMsg2); if(Failed()) return
      p%TIreact_XSEA=transpose(T_TIreact_XSEA)
      deallocate(T_TIreact_XSEA)
...
SUBROUTINE SDOut_MapOutputs(u,p,x, y, m, AllOuts, ErrStat, ErrMsg )
   ...
   real(ReKi), dimension (12)     :: FK_elm_force      !output static and dynamic forces and moments
   real(ReKi), dimension (6)      :: FK_elm_OUT      !output static and dynamic forces and moments
   ...
   integer(IntKi)                          :: NELEMENT 
   integer(IntKi)                          :: NNODE
   integer(IntKi)                          :: NOPTION_INITIAL
   integer(IntKi)                          :: NALLOWPRINT_INITIAL
   INTEGER(IntKi)                          :: FILENUMBERINTER 
   INTEGER(IntKi)                          :: n_t_global
   INTEGER(IntKi)                          :: IINDEX_RE
   real(ReKi)                              :: ReactNs_IN 
   CHARACTER(600)                          :: MAKEDIROUTFILE
   CHARACTER(600)                          :: MAKEDIROUTFILE_MEM
   CHARACTER(600)                          :: FILENUMBER_TEXT
   
   COMMON /TOTAL_TIME/n_t_global
   COMMON /FASTCONTROL_PARA/ FILENUMBERINTER
   COMMON /OUTPUT_XSEA/ NOPTION_INITIAL,NALLOWPRINT_INITIAL
   ErrStat = ErrID_None   
   ErrMsg  = ""
   
   AllOuts = 0.0_ReKi  ! initialize for those outputs that aren't valid (and thus aren't set in this routine)
500 FORMAT ("GiD Post Results File 1.0")
501 FORMAT ("")
502 FORMAT ('GaussPoints "XFrame" ElemType Linear')
503 FORMAT ("Number of Gauss Points:    5")
504 FORMAT ("Natural Coordinates: Internal")
505 FORMAT ("Nodes Included")
506 FORMAT ("End GaussPoints")
507 FORMAT ("")
508 FORMAT ('GaussPoints "XFrame2" ElemType Linear')
509 FORMAT ("Number of Gauss Points:    2")
510 FORMAT ("Natural Coordinates: Internal")
511 FORMAT ("Nodes Included")
512 FORMAT ("End GaussPoints")
513 FORMAT ("")
514 FORMAT ('GaussPoints "XWave" ElemType Linear')
515 FORMAT ("Number of Gauss Points:    2")
516 FORMAT ("Natural Coordinates: Internal")
517 FORMAT ("Nodes Included")
518 FORMAT ("End GaussPoints")  
519 FORMAT ("")
520 FORMAT ("")
800 FORMAT ("End Values")
    
   IF (NOPTION_INITIAL.EQ.0.AND.NALLOWPRINT_INITIAL.EQ.1)THEN
      OPEN(UNIT=1040  ,  FILE="integer_to_string.DAT"               ,STATUS='UNKNOWN')
      REWIND (1040)
      WRITE (1040,700) FILENUMBERINTER
700   FORMAT (I5)
      REWIND (1040)
      READ (1040,*) FILENUMBER_TEXT
      CLOSE (1040, status='delete')
      
    MAKEDIROUTFILE   = "gidout"//TRIM(FILENUMBER_TEXT)//".flavia.res"  
    OPEN(UNIT=7100  ,  FILE=MAKEDIROUTFILE               ,STATUS='UNKNOWN')
    MAKEDIROUTFILE_MEM   = "list_of_member_force"//TRIM(FILENUMBER_TEXT)//".dat"  
    OPEN(UNIT=7101  ,  FILE=MAKEDIROUTFILE_MEM               ,STATUS='UNKNOWN')
    ! HEADER
      WRITE (7100,500) 
      WRITE (7100,501) 
      WRITE (7100,502) 
    ...
    ! MEMBER FORCE
      WRITE (7100,200) FILENUMBERINTER
      WRITE (7100,201) 
      WRITE (7100,202)
    DO NELEMENT = 1,p%NMEMBERS
        pLst_XSEA=>p%MOutLst_XSEA(NELEMENT) ! List for a given member-output 
        DO NNODE = 1,2
        call ElementForce_XSEA(NELEMENT,pLst_XSEA, NNODE, 1, FM_elm, FK_elm, sgn, DIRCOS, .false.)
        FK_elm2=sgn*FK_elm
        !IF (NNODE.EQ.1) FK_elm_force (1:6) = FK_elm2
        !IF (NNODE.EQ.1) FK_elm_force (7:12) = FK_elm2
        IF (NNODE.EQ.1) WRITE (7100,100) NELEMENT,FK_elm2(1),FK_elm2(2),FK_elm2(3),FK_elm2(4),FK_elm2(5),FK_elm2(6)
        IF (NNODE.EQ.2) WRITE (7100,101) FK_elm2(1),FK_elm2(2),FK_elm2(3),FK_elm2(4),FK_elm2(5),FK_elm2(6)
        FK_elm = 0.
        FK_elm2 = 0.
        ENDDO
    !    IF (ABS(FK_elm_force(1)).GT.ABS(FK_elm_force(7))) FK_elm_OUT(1) = FK_elm_force(1)
    !    IF (ABS(FK_elm_force(1)).LE.ABS(FK_elm_force(7))) FK_elm_OUT(1) = FK_elm_force(7)
    !    IF (ABS(FK_elm_force(2)).GT.ABS(FK_elm_force(8))) FK_elm_OUT(2) = FK_elm_force(2)
    !    IF (ABS(FK_elm_force(2)).LE.ABS(FK_elm_force(8))) FK_elm_OUT(2) = FK_elm_force(8)
    !    IF (ABS(FK_elm_force(3)).GT.ABS(FK_elm_force(9))) FK_elm_OUT(3) = FK_elm_force(3)
    !    IF (ABS(FK_elm_force(3)).LE.ABS(FK_elm_force(9))) FK_elm_OUT(3) = FK_elm_force(9)
    !    IF (ABS(FK_elm_force(4)).GT.ABS(FK_elm_force(10))) FK_elm_OUT(4) = FK_elm_force(4)
    !    IF (ABS(FK_elm_force(4)).LE.ABS(FK_elm_force(10))) FK_elm_OUT(4) = FK_elm_force(10)
    !    IF (ABS(FK_elm_force(5)).GT.ABS(FK_elm_force(11))) FK_elm_OUT(5) = FK_elm_force(5)
    !    IF (ABS(FK_elm_force(5)).LE.ABS(FK_elm_force(11))) FK_elm_OUT(5) = FK_elm_force(11)
    !    IF (ABS(FK_elm_force(6)).GT.ABS(FK_elm_force(12))) FK_elm_OUT(6) = FK_elm_force(6)
    !    IF (ABS(FK_elm_force(6)).LE.ABS(FK_elm_force(12))) FK_elm_OUT(6) = FK_elm_force(12)
    !WRITE (7100,100) NELEMENT,FK_elm_OUT(1),FK_elm_OUT(2),FK_elm_OUT(3),FK_elm_OUT(4),FK_elm_OUT(5),FK_elm_OUT(6)
    !WRITE (7100,101) FK_elm_OUT(1),FK_elm_OUT(2),FK_elm_OUT(3),FK_elm_OUT(4),FK_elm_OUT(5),FK_elm_OUT(6)
    FK_elm_force = 0.
    ENDDO
    WRITE (7100,800)
    WRITE (7100,203)
    ...
    WRITE (7100,210) FILENUMBERINTER
    WRITE (7100,211) 
    WRITE (7100,212)
      
    DO iiNode = 1,p%NNODES     
    DOFList => p%NodesDOF(iiNode)%List
    WRITE (7100,100) iiNode,m%U_full(DOFList(1)),m%U_full(DOFList(2)),m%U_full(DOFList(3)),m%U_full(DOFList(4)),m%U_full(DOFList(5)),m%U_full(DOFList(6))
    ENDDO
    WRITE (7100,800)
    WRITE (7100,213)
    WRITE (7100,213)
    ...
    ! REACTION
    WRITE (7100,220) FILENUMBERINTER
    WRITE (7100,221) 
    WRITE (7100,222) 
    ...
    ReactNs_IN = 0.0D0
      DO I=1,p%nNodes_C   !Do for each constrained node, they are ordered as given in the input file and so as in the order of y2mesh
         ...
         DO J=1,SIZE(pLst_XSEA%ElmIDs(1,:))  !for all the elements connected (normally 1)
            iiNode = 1
            call ElementForce(pLst_XSEA, iiNode, J, FM_elm, FK_elm, sgn, DIRCOS, .false.)
      ...
      ENDDO
       ...
       DO iiNode = 1,p%NNODES     
           IINDEX_RE = 0.00D0
          DO 600 I = 1,p%nNodes_C  
            IF (p%MOutLst3_XSEA(I)%NOUTCNT.eq.iiNode)THEN
            IINDEX_RE = I
            EXIT
            ENDIF
600       CONTINUE
       IF (IINDEX_RE.EQ.1) THEN
       WRITE (7100,100) iiNode,ReactNs(1),ReactNs(2),ReactNs(3),ReactNs(4),ReactNs(5),ReactNs(6)
       ELSEIF (IINDEX_RE.GT.1)THEN
       WRITE (7100,100) iiNode,ReactNs(((IINDEX_RE-1)*6)+1),ReactNs(((IINDEX_RE-1)*6)+2),ReactNs((IINDEX_RE-1)*6)+(3),ReactNs(((IINDEX_RE-1)*6)+4),ReactNs(((IINDEX_RE-1)*6)+5),ReactNs(((IINDEX_RE-1)*6)+6)
       ELSEIF (IINDEX_RE.EQ.0)THEN
       WRITE (7100,100) iiNode,ReactNs_IN,ReactNs_IN,ReactNs_IN,ReactNs_IN,ReactNs_IN,ReactNs_IN 
       ENDIF
       ENDDO
    WRITE (7100,800)   
    WRITE (7100,223)
    ...
    NALLOWPRINT_INITIAL = 0D0
        
        
   ELSEIF (NOPTION_INITIAL.EQ.2.AND.NALLOWPRINT_INITIAL.EQ.1)THEN
   ! MEMBER FORCE
      WRITE (7100,204) FILENUMBERINTER,n_t_global+2
      WRITE (7100,201) 
      WRITE (7100,202)
    DO NELEMENT = 1,p%NMEMBERS
        pLst_XSEA=>p%MOutLst_XSEA(1) ! List for a given member-output 
        DO NNODE = 1,2
        call ElementForce_XSEA(NELEMENT,pLst_XSEA, NNODE, 1, FM_elm, FK_elm, sgn, DIRCOS, .false.)
        FK_elm2=sgn*FK_elm
        !IF (NNODE.EQ.1) FK_elm_force (1:6) = FK_elm2
    ...
    ! DISPLACEMENT AND ROTATION
    WRITE (7100,214) FILENUMBERINTER,n_t_global+2
    WRITE (7100,211) 
    WRITE (7100,212)
    ...
    ! REACTION
    WRITE (7100,224) FILENUMBERINTER,n_t_global+2
    WRITE (7100,221) 
    WRITE (7100,222) 
    ALLOCATE ( ReactNs(6*p%nNodes_C), STAT = ErrStat )
    ReactNs = 0.0_ReKi !Initialize
...
100     FORMAT (I5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5)
101     FORMAT (7X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5,2X,E12.5)
   
200 FORMAT ('Result "OpenFAST-Local-Force"  "Load Case',I2,'"      1  Matrix  OnGaussPoints  "XFrame2"')
201 FORMAT ('ComponentNames "Fx" "Fy" "Fz" "Mx" "My" "Mz"')
202 FORMAT ('Values')
203 FORMAT ("")
204 FORMAT ('Result "OpenFAST-Local-Force"  "Load Case',I2,'"',2x,I5,2X,'Matrix  OnGaussPoints  "XFrame2"')
    

210 FORMAT ('Result "Displacement and Rotation"  "Load Case',I2,'"      1  Matrix  OnNodes')
211 FORMAT ('ComponentNames "U" "V" "W" "Rx" "Ry" "Rz"')
212 FORMAT ('Values')
213 FORMAT ("")
214 FORMAT ('Result "Displacement and Rotation"  "Load Case',I2,'"',2x,I5,2X,'Matrix  OnNodes')
    
    

220 FORMAT ('Result "Reaction"  "Load Case',I2,'"      1  Matrix  OnNodes')
221 FORMAT ('ComponentNames "F-X" "F-Y" "F-Z" "M-X" "M-Y" "M-Z"')
222 FORMAT ('Values')
223 FORMAT ("")
224 FORMAT ('Result "Reaction"  "Load Case',I2,'"',2x,I5,2X,'Matrix  OnNodes')
...
contains
   ...
   subroutine ElementForce(pLst, iiNode, JJ, FM_elm, FK_elm, sgn, DIRCOS, bUseInputDirCos)
      ...
      subroutine ElementForce_XSEA(NELEMENT,pLst_XSEA, iiNode, JJ, FM_elm, FK_elm, sgn, DIRCOS, bUseInputDirCos)
      type(MeshAuxDataType), intent(in)          :: pLst_XSEA   !< Info for one member output
      integer(IntKi)       , intent(in)          :: iiNode !< Index over the nodes of a given member (>2 if nDIV>1)
      integer(IntKi)       , intent(in)          :: JJ     !< TODO: interpretation: index over other member connected to the current member (for averaging)
      real(FEKi), dimension (3,3), intent(inout) :: DIRCOS  !direction cosice matrix (global to local) (3x3)
      real(ReKi), dimension (6), intent(out)     :: FM_elm, FK_elm  !output static and dynamic forces and moments
      ...
      CALL CALC_NODE_FORCES( DIRCOS, pLst_XSEA%Me(:,:,iiNode,JJ),pLst_XSEA%Ke(:,:,iiNode,JJ), Xdd_e, X_e, pLst_XSEA%Fg(:,iiNode,JJ), FirstOrSecond, FM_elm, FK_elm) 
   end subroutine ElementForce_XSEA
