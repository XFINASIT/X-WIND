...
!   Results
    ...
    COMPLEX,DIMENSION(:,:),ALLOCATABLE :: Force_MESH
    REAL,DIMENSION(:),ALLOCATABLE :: line
    REAL,DIMENSION(:),ALLOCATABLE :: line_MESH
!   Locals
    REAL                    :: PI
    ...
    OPEN(UNIT=1000,FILE='screenjob_solver.dat',STATUS='UNKNOWN')
!
!   --- Initialisation -------------------------------------------------------------------------------------------------------------------------------------------------------------------
!     
    PI=4.*ATAN(1.)
...
!   Initialise results table
    ...
    ALLOCATE(Force_MESH(Mesh%Npanels*2**Mesh%Isym,Bodyconditions%Nproblems))
    Force(:,:)=0.
    Force_MESH = 0.
    WRITE(*,*) '. Done !'
    WRITE(*,*) ' '
...
!
    ...
    WRITE(*,*) ' '
    WRITE(1000,*) ' -> Solve BVPs and calculate forces ' 
    WRITE(1000,*) ' '
    DO j=1,BodyConditions%Nproblems 
        WRITE(*,'(A,I5,A,I5,A,$)') ' Problem ',j,' / ',BodyConditions%Nproblems,' .'
        WRITE(1000,'(A,I5,A,I5,A,$)') ' Problem ',j,' / ',BodyConditions%Nproblems,' .'
...
!       Calculate force coefficients
        ...
        DO c=1,Mesh%Npanels*2**Mesh%Isym
            Force_MESH(C,j)=Force_MESH(C,j)-PRESSURE(C)
        END DO
...
!
    ...
    WRITE(1000,*) ' -> Save results ' 
    WRITE(1000,*) ' '
    OPEN(10,FILE=ID%ID(1:ID%lID)//'/results/Forces_PANNEL.dat')
    ALLOCATE(line_MESH(BodyConditions%Nproblems*2))
    DO c=1,Mesh%Npanels*2**Mesh%Isym
        DO j=1,BodyConditions%Nproblems
            IF (Bodyconditions%Switch_type(j).NE.-1.) THEN
                line_MESH(2*j-1)=ABS(Force_MESH(c,j))
                line_MESH(2*j)=ATAN2(IMAG(Force_MESH(c,j)),REAL(Force_MESH(c,j)))
            ELSE
                line_MESH(2*j-1)=IMAG(Force_MESH(c,j))/BodyConditions%Omega(j)
                line_MESH(2*j)=-REAL(Force_MESH(c,j))
            END IF
        END DO
        WRITE(10,*) (line_MESH(j),j=1,2*BodyConditions%Nproblems)      
    END DO
    CLOSE(10)    
!
!   --- Finalize -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
!
    ...
    DEALLOCATE(Force)
    DEALLOCATE(Force_MESH)
    DEALLOCATE(Theta,HKochin)
    DEALLOCATE (line_MESH)
