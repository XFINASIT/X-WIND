...
!   Froude Krylov forces
    ...
    COMPLEX,DIMENSION(:,:,:),ALLOCATABLE :: FKforce_MESH
!   Free surface visualisation
    INTEGER :: Switch_FreeSurface
    INTEGER :: Nx,Ny
    INTEGER :: II,INDEX_LINE
    REAL :: Lx,Ly
!   Kochin function
...
!   Other local variables
    ...
    OPEN(UNIT=1000,FILE='screenjob_preprocess.dat',STATUS='UNKNOWN')
!
    PI=4.*ATAN(1.)
!
...
!   Print summary of calculation case
    ...
    WRITE(*,*) ' '
    WRITE(1000,*) ' '
    WRITE(1000,*) ' Summary of calculation'
    WRITE(1000,*) ' '
    IF (Environment%Depth.GT.0.) THEN
        WRITE(*,'(A,F7.2,A)') '  ->  Water depth = ',Environment%Depth,' m'
        WRITE(1000,'(A,F7.2,A)') '  ->  Water depth = ',Environment%Depth,' m'
    ELSE
        WRITE(1000,'(A)') '  ->  Infinite water depth'
        WRITE(1000,'(A)') '  ->  Infinite water depth'
    END IF
    ...
    WRITE(*,*) ' '
    WRITE(1000,'(A,I5,A,F7.4,A,F7.4)') '  ->',Nw,' wave frequencies from ',w(1),' to ',w(Nw)
    WRITE(1000,'(A,I5,A,F7.4,A,F7.4)') '  ->',Nbeta,' wave directions from  ',beta(1),' to ',beta(Nbeta)
    WRITE(1000,'(A,I5,A)') '  ->',Nradiation,' radiation problems'
    WRITE(1000,'(A,I5,A)') '  ->',Nintegration,' forces'
    WRITE(1000,*) ' '
!
...
!
    ...
    ALLOCATE(FKForce_MESH(Nw,Nbeta,Mesh%Npanels)) ! MID POINT
...
!           Calculate the corresponding FK forces
            DO k=1,Nintegration
                ...
                END DO
            END DO
            DO k=1,Mesh%nPanels*2**Mesh%Isym
            FKForce_MESH(i,j,k)=FKForce_MESH(i,j,k)-PRESSURE(K)!*FNDS(k,c)      
            ENDDO
        END DO
        DO j=1,Nradiation
...
!
    ...
    OPEN(10,FILE=ID%ID(1:ID%lID)//'/results/FKForce_PANNEL.tec')
    !WRITE(10,'(A)') 'VARIABLES="w (rad/s)"'
    !DO k=1,Nintegration
    !    WRITE(10,'(A,I4,I4,A,I4,I4,A)') '"abs(F',IntCase(k)%Body,k,')" "angle(F',IntCase(k)%Body,k,')"'
    !END DO
    DO c=1,Nbeta
        WRITE(10,'(A,F7.3,A,I6,A)') 'Zone t="FKforce - beta = ',beta(c)*180./PI,'",I=',Nw,',F=POINT'
        DO i=1,Nw
           ! AAAA = ATAN2(IMAG(FKForce(2,1,k)),REAL(FKForce(2,1,k)))
           !AAAC = ABS(FKForce(2,1,k))
            WRITE(10,'(80(X,E14.7))') w(i),(ABS(FKForce_MESH(i,c,k)),ATAN2(IMAG(FKForce_MESH(i,c,k)),REAL(FKForce_MESH(i,c,k))),k=1,Mesh%Npanels)
        END DO
    END DO
    CLOSE(10)
    
    OPEN(10,FILE=ID%ID(1:ID%lID)//'/results/FKForce_PANNEL.out')
    DO c=1,Nbeta
        WRITE(10,'(A,F7.3,A,I6,A)') 'Zone t="FKforce - beta = ',beta(c)*180./PI,'",I=',Nw,',F=POINT'
        DO i=1,Nw
            DO k=1,Mesh%Npanels
            WRITE(10,500) w(i),REAL(FKForce_MESH(i,c,k)),IMAG(FKForce_MESH(i,c,k)),ATAN2(IMAG(FKForce_MESH(i,c,k)),REAL(FKForce_MESH(i,c,k)))
500         FORMAT (E14.7,2X,E14.7,2X,E14.7,2X,E14.7)
            ENDDO
            END DO
    END DO
    CLOSE(10)
    ...
    OPEN(10,FILE=ID%ID(1:ID%lID)//'/results/FKForce_PANNEL.dat')
    DO k=1,Mesh%Npanels
        WRITE(10,*) ((ABS(FKForce_MESH(i,c,k)),ATAN2(IMAG(FKForce_MESH(i,c,k)),REAL(FKForce_MESH(i,c,k))),c=1,Nbeta),(0.*c,0.*c,c=1,Nradiation),i=1,Nw) 
    END DO
    CLOSE(10)
    
    DEALLOCATE(FKForce,FKForce_MESH)
!
