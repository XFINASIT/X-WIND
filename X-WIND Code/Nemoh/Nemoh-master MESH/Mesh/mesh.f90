...
!
    ...
    OPEN(UNIT=1000,FILE='screenjob_mesh.dat',STATUS='UNKNOWN')

	WRITE(*,*)
	WRITE(*,*) ' -> Read input data '
	WRITE(*,*) ' '
	WRITE(1000,*)
	WRITE(1000,*) ' -> Read input data '
	WRITE(1000,*) ' '
	...
	DO i=1,Np
	    READ(10,*) X(i),Y(i),Z(i)
        IF (X(i).LE.0.000001D0.AND.X(i).GE.-0.000001D0) X(i) = 0.D0
        IF (Y(i).LE.0.000001D0.AND.Y(i).GE.-0.000001D0) Y(i) = 0.D0
        IF (Z(i).LE.0.000001D0.AND.Z(i).GE.-0.000001D0) Z(i) = 0.D0
	END DO
	DO j=1,nFace
...
!   Mise a l echelle
    IF ((lambda).NE.(1.)) THEN
        ...
        WRITE(1000,'(A,F7.3)') '   - Mesh is scaled by ', lambda
        xG=xG*lambda
        yG=yG*lambda
        zG=zG*lambda
        tX=tX*lambda
...
!   Translate
    ...
    WRITE(*,*) ' '     
    WRITE(1000,*) ' -> Calculate hydrostatics '
    WRITE(1000,*) ' ' 
	Np=0
	Nf=nFace
	DO i=1,nFace
...
!	Prise en compte de la symétrie
	...
	WRITE(*,*) ' '
    WRITE(1000,'(A,I3)') '   - Coordinates of buoyancy centre '
	WRITE(1000,'(A,F7.3,A)') '     XB = ',XF+xG,'m'
	WRITE(1000,'(A,F7.3,A)') '     YB = ',YF+yG,'m'
	WRITE(1000,'(A,F7.3,A)') '     ZB = ',ZF,'m'
	WRITE(1000,'(A,E14.7,A)') '    - Displacement  = ',DEPLACEMENT,' m^3'
	WRITE(1000,'(A,E14.7,A)') '    - Waterplane area  = ',SF, ' m^2'
	WRITE(1000,*) ' '
	IF ((ABS(XF).GT.1E-02).OR.(ABS(YF).GT.1E-02)) THEN
	    ...
	    WRITE(1000,*) ' !!! WARNING !!! '
		WRITE(1000,*) ' '
		WRITE(1000,'(A,I3)') ' Buoyancy center and gravity center are not vertically aligned. '
		WRITE(1000,*) ' This is not an equilibrium position.'
		WRITE(1000,'(A,F7.3,1X,A,F7.3)') ' XF = ',XF+xG,' XG = ',xG
		WRITE(1000,'(A,F7.3,1X,A,F7.3)') ' YF = ',YF+yG,' YG = ',yG
	END IF
...
!	WRITE(*,*) ' '
    ...
    ! WRITE(1000,'(A,I5)') '    Number of panels in final mesh : ',NF
...
!	Calcul et sauvegarde de la matrice de raideur hydrostatique
	...
	WRITE(*,*) ' '
    write(1000,*) ' -> Calculate hull mass and inertia '
	WRITE(1000,*) ' '
	CDG(1)=xG
	CDG(2)=yG
	CDG(3)=zG	
